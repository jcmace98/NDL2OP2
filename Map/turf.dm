		...
		emptytop
			...
			verb
				Rest()
					...
					if(usr.shari||usr.bya)
						usr<<"Your Doujutsu prevents you from resting at the moment. Turn it off.";usr.CanRest=1;return
					else
						usr.resting=1
						...
						while(usr.resting)
							...
							if(usr.shari||usr.bya)
								usr.shari=0
								usr.bya=0
								usr<<"The liquid in the tank  causes you to relax your eyes, releasing your Doujutsu."
							sleep(11)
		...
		Enter(A)
			if(ismob(A))
				...
				if(!M.client)
					...
					if(usr.sevenswordsmenmember)
						if(usr.Yonkami)
							usr<<"nop"
							return 0
						usr.loc = locate(19,161,20)
					else
						var/Q=input(usr,"What is the password?") as text
						if(Q=="BitchThisIs7SM")
							if(usr.Yonkami)
...
//////
		...
		MistSign2
			icon = 'Scenary.dmi'
			icon_state="MistSign3"
			layer=TURF_LAYER
		MistSign3
			icon = 'Scenary.dmi'
			icon_state="MistSign2"
			layer=MOB_LAYER+4
			...
			if(ismob(M))
				...
				//		Chance+=10
					...
					if(prob(Chance))
						...
						spawn(15)
							src.overlays-=I
						M.WaterWalkingMastery+=pick(0.1,0.2)
						if(M.WaterWalkingMastery>10)
							M.WaterWalkingMastery=10
				if(prob(1))
					usr.ChakraC++
					if(usr.ChakraC>100&&usr.Clan!="Fuuma"||usr.ChakraC>100&&usr.Clan2!="Fuuma")
						usr.ChakraC=100
					if(usr.ChakraC>=160&&usr.Clan=="Fuuma"||usr.ChakraC>=160&&usr.Clan2=="Fuuma")
						usr.ChakraC=160
				if(M.chakra<=0)
					M<<"<b>You ran out of chakra!"
					M.stamina-=rand(M.stamina,M.stamina/100);return
...
obj
	Trees1
		...
		DEADgrowstump
			...
			icon_state="tree10"
			New()
				src.overlays+=/obj/Trees1/DEADgrowstump
