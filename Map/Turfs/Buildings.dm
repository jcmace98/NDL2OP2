...
turf/Buildings/Konoha/Hospital
	...
	layer=MOB_LAYER+1
turf/Buildings/Rain/SharkHouse
	icon='SharkHouse.png'
	layer=MOB_LAYER+1
turf/Batruzon
...
turf/Buildings/Teleporters
	...
	OutofSharkHouse
		...
		Enter(A)
			if(ismob(A))
				var/mob/M = A
				M.loc=locate(182,181,20)
			else if(isobj(A))
				var/obj/B=A
				B.loc=locate(182,181,20)
	ToCastle1
		density=0
		Enter(A)
			if(ismob(A))
...
turf/Dojos/LeafDojo
	ToLeafDojo
		...
		/*		var/BreakingIn=0
				...
				var/AlertVillage=0
				var/Invader=""
			//	if((M.TaijutsuKnowledge+M.GenjutsuKnowledge+M.NinjutsuKnowledge<1600||M.rank=="Hokage"||M.key=="")&&(M.rank=="Genin"||M.rank=="Chuunin"||M.rank=="S.Jounin"||M.rank=="Hokage")&&M.Village=="Leaf")
				if(((M.rank=="Genin"&&M.Village=="Leaf")||M.rank=="Hokage"||M.key=="")||LeafOnlyGeninDojo==0)
