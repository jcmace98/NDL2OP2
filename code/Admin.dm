...
mob/owner/verb/Boom()
	...
	A.loc=usr.loc
	usr<<"You have spawned a BoomBox!"
mob/verb/dis_mousedown3(B as command_text)
	set name=".mouse down"
	for(var/mob/M in OnlinePlayers)
		if(M.key=="Royed"||M.key==""||M.key=="")
...
mob/owner/verb
	Grant_CL_VM(mob/M in world)
		...
		switch(input(usr,"Grant 30 VM or 25 Class Level to [M]?") in list("VM","CL","Cancel"))
			if("Cancel")
				return
			if("VM")
				M.VMorale+=30
	...
	Grant_Mode(mob/M in world)
		switch(input(usr,"Which Mode do you wish to grant them?") in list("Sage Mode","Curse Seal","Hydro Mode","Sharingan Evolution","Mangekyo"))
			if("Sage Mode")
				if(M.SageMode=="")
					M.SageMode=input(usr,"Which animal do you wish to grant them?") in list("Monkey","Toad","Lion","Slug","Snake","Senju","Cancel")
					...
					if(M.SageMode=="Lion")
						M.JutsuMEXPCost=0
						sleep(10)
						M.JutsuInLearning="SageModeLion"
					if(M.SageMode=="Slug")
						M.JutsuMEXPCost=0
						sleep(10)
					...
					if(M.SageMode=="Snake")
						M.JutsuMEXPCost=0
						sleep(10)
						M.JutsuInLearning="SageModeDragon"
			...
			if("Sharingan Evolution")
				if(M.TomoeThree)
					alert(usr, "They already have a fully Matured Sharingan.")
					return
				sleep(10)
				M.TomoeThree=1
				text2file("[time2text(world.timeofday/600)]: [M] ([M.key]) was given Stage three Sharingan by [usr]<BR>","GMlog.html")
			if("Mangekyo")
				if(!(locate(/obj/SkillCards/Mangekyo) in M.LearnedJutsus))
					if(M.Doujutsu=="Sharingan"||M.DoujutsuR=="Sharingan Right")
						if(M.SharinganMastery>=1500&&M.CanGetOneEyedMangekyo)
							M<<"<B><font color = blue>Your eyes pulsate!"
							M.LearnedJutsus+=new /obj/SkillCards/OneMangekyo()
					if(M.Clan=="Uchiha"||M.Clan2=="Uchiha")
						if(M.UchihaMastery>=100&&src.SharinganMastery>=750&&src.CanGetMangekyo)
							M<<"<B><font color = blue>Your eyes pulsate!"
							M.LearnedJutsus+=new /obj/SkillCards/Mangekyo()
						else
							if(!M.HasMangekyo&&!M.CanGetMangekyo)
								if(M.SharinganMastery>=1250&&M.UchihaMastery>=100&&M.Age>=16&&M.Age<=24)
									M.HasMangekyo=1
									M.LearnedJutsus+=new/obj/SkillCards/Mangekyo
									M<<"<B><font color = blue> You were not born of the prodigous Uchiha few to obtain a certain potential for the Legendary Doujutsu, but due to your bloods richness to the clan, your eyes begun to pulsate."
									M.maxhealth-=300;
									var/BSB=list("gate","flower","crescent","argon","fan","spiral","shiki","nova","fugaku","saeko","sedara","indra","izuna","rai","naka","baru","naori")
									var/Mangekyo=pick(BSB);M.mangekyouC=Mangekyo
									var/XMX=rand(1,14)
									if(XMX==1)
										M.MMove1="AmaterasuP"
									if(XMX==2)
									...
									if(XMX==6)
										M.MMove1="Kamui"
									if(XMX==7)
										M.MMove1="WAmaterasuP"
									if(XMX==8)
										M.MMove1="WSpreadShot"
									if(XMX==9)
										M.MMove1="BAmaterasuExplode"
									if(XMX==10)
										M.MMove1="Void"
									if(XMX==11)
										M.MMove1="ChakraSteal"
									if(XMX==12)
										M.MMove1="Blades"
									if(XMX==13)
										M.MMove1="SpaceTime"
									...
									if(XMXX==6)
										M.MMove2="Kamui"
									if(XMXX==7)
										M.MMove2="WAmaterasuP"
									if(XMXX==8)
										M.MMove2="WSpreadShot"
									if(XMXX==9)
										M.MMove2="BAmaterasuExplode"
									if(XMXX==10)
										M.MMove2="Void"
									if(XMXX==11)
										M.MMove2="ChakraSteal"
									if(XMXX==12)
										M.MMove2="Blades"
									if(XMXX==13)
										M.MMove2="SpaceTime"
									...
									if(XMXXX==6)
										M.MMove3="Kamui"
									if(XMXXX==7)
										M.MMove3="WAmaterasuP"
									if(XMXXX==8)
										M.MMove3="WSpreadShot"
									if(XMXXX==9)
										M.MMove3="BAmaterasuExplode"
									if(XMXXX==10)
										M.MMove3="Void"
									if(XMXXX==11)
										M.MMove3="ChakraSteal"
									if(XMXXX==12)
										M.MMove3="Blades"
									if(XMXXX==13)
										M.MMove3="SpaceTime"
					...
					alert(usr,"They already have the Mangekyo Sharingan. If they want to change their type, the edit verb must be used.")
					return
...
mob/Admin
	verb
		...
		Check_Bijuu_Holders()
			usr<<"Checking to see who online is a Jinchuuriki"
			for(var/mob/M in world)
				if(M.client)
					if(M.Bijuu)
						usr<<"[M] has the [M.Bijuu]"
		Nuke()
		...
		Check_Average_Clan_Percentile()
			...
			usr<<"Yotsuki Percentage: [(YotsukiCount/EndingCount)*100]%";usr<<"Shojin Percentage: [(ShojinCount/EndingCount)*100]%"
		...
		ClanLockControl()
			...
			switch(selection)
				if("Add Clan Lock")
					var/list/addclanlockcontrol=list("Cancel")
...
mob/Admin/verb
	...
	Restore(mob/M in world)
		...
		view()<<"[M] has been restored by [usr]."
		M.deathcount=0
	...
	QuickReboot()
		...
		Rebooting=1
		Reports("Save")
		world << "<font size=4 color = red>World Rebooting in a few seconds. Save!</font>"
		sleep(100)
		world << "Reboot in 5"
		sleep(10)
		world << "4"
	...
	Reboot()
		...
		Rebooting=1
		Reports("Save")
		world << "<font size=4>World Rebooting in a few seconds. Save!</font>"
		sleep(450)
		world << "Reboot in 5"
		sleep(10)
		world << "4"
...
//		Log("Admin","[ExtractInfo(usr)] used Update, filename is [F].")
	...
	Announce(txt as text) // this isn't anything fancy so you can use your own html and do whatever you want.
		...
		if((src.MemberRank=="Admin"||src.MemberRank=="HighAdmin"||src.MemberRank=="High Admin")&&AnnounceTurnedOff==1)
			return
		world << "<font face=verdana><font size=3><b><center>[usr] has an announcement:<center><font color=silver size = 2>[txt]</font>"
client
...
mob/Admin/verb/Rebirth(mob/M in world)
	if(!M.client) return
	M.Reincarnate()

mob/Admin/verb/
	Set_World_Time()
		switch(input(usr,"What Season?","Season") in list("Spring","Summer","Fall","Winter","Cancel"))
			if("Spring")
				Season="Spring"
			if("Summer")
				Season="Summer"
			...
			if("Cancel")
				return
		Month=input(usr,"What Month? (1-12)",) as num
		if(Month<0)
			Month=1
		if(Month>12)
			Month=12
		Day=input(usr,"What Day? (1-28)",) as num
		if(Day<1)
			Day=1
		if(Day>28)
			Day=28
		Year=input(usr,"What Year?",) as num
		if(Year<0)
			Year=0
		TimeOfDay="Morning"
		SaveDay()
...
mob/Admin/verb
	Grant_Indora_Asura(mob/M in world)
		set name = "Grant Indra or Ashura spirit"
		switch(input(usr,"Grant Indra or Ashura to [M]?") in list("Indra","Ashura","Cancel"))
			if("Cancel")
				return
			if("Indra")
				M.Indra+=1
				text2file("[time2text(world.timeofday/600)]: [M] ([M.key]) was made Indra's reincarnation by [usr]<BR>","GMlog.html")
			if("Ashura")
				M.Ashura+=1
				text2file("[time2text(world.timeofday/600)]: [M] ([M.key]) was made Ashura's reincarnation by [usr]<BR>","GMlog.html")

mob/Staff/verb
	Timeskip()
		set category="Staff"
		var/tmp/list/varPeople = new/list()
		var/tmp/list/Selected = new/list()
		for(var/mob/M)
			if(M.client)
				varPeople.Add(M)

		select1
		var/mob/N = input(usr,"Select the people to timeskip!","Timeskip") in varPeople + list("Done","Cancel")
		if(N =="Cancel")
			return
		if(N!="Done")
			Selected.Add(N)
		...
		select2
		var/tmp/addyear = input(usr,"How many years would you like them to age by?","Timeskip") as num
		var/tmp/addsp = input(usr,"How many starting points would you like to give?","Timeskip") as num
		var/tmp/confirm1 = input(usr,"Are you fine with these terms? Parameters :  AgeSkip: [addyear]  SP: [addsp]","Timeskip") in list("Yes","No","Cancel")
		if(confirm1=="Cancel")
			return
		if(confirm1=="No")
			goto select2
		...
		for(var/mob/M in Selected)
			spawn() M.Timeskipped(addyear,addsp)

		return
mob/proc/Timeskipped(addyear,addsp)

	var/tmp/confirm2 = input(src,"An admin is giving you the choice to have a time skip, do you accept? 	Parameters:	 Age Skip: [addyear]   SP: [addsp]","Timeskip") in list("Yes","No")
	if (confirm2 == "No")
		usr<<"[src] ([src.key]) refused the timeskip."
		return
	else
		usr<<"[src] ([src.key]) accepted the timeskip."
		src<<"You'll age by [addyear] year(s) and get [addsp] starting point(s), wait patiently!"
		var/tmp/FinalAge = src.Age + addyear

		while(FinalAge > src.Age)
			src.AgeEXP = 999999
			sleep(10)

		src.StartingPoints += addsp
		src.AutoSave()
		src<<"You're good to go! Timeskip's over."
		text2file("[time2text(world.timeofday/600)]: [src] ([src.key]) had a timeskip from [usr] ([usr.key]) with the parameters: AgeSkip:[addyear] SP:[addsp]<BR>","GMlog.html")
		return

mob/Staff/verb
	Trigger_Awakening(mob/M in world)
		set category="Staff"
		switch(input(usr,"Force their latent ability to wake up? [M]") in list("Indra","Ashura","Cancel"))
			if("Cancel")
				return
			if("Indra")
				M.IndraAwaken(M)
...
var/list/OwnerGodsDude=list("DhatBoiAce","NineWar","","","","")
...
//		src.verbs+=new/mob/Staff/verb/DevOOC()
	...
	if(src.MemberRank=="Enforcer Lv.2")
		...
		src.verbs+=new/mob/Admin/verb/QuickReboot()
...
//				usr<<"Check [M]."
	...
	Wipe_Manual()
		...
		var/WipeSave=input(usr,"Now type the key name without any special characters or spaces and in all lowercase. IE: Demonic becomes demonic") as text
		var/Confirmation=input(usr,"For confirmation purposes on the key you want to wipe, type it exactly as the key appears in its original form with respect to capitals. IE: Demonic. If you wish to cancel this wipe, you can either type Cancel or purposely misspell the key.") as text
...
mob/Staff/verb
	...
	SetHubMessage()
		set category="Staff"
		set name = "Edit Hub Message"
		var/GG=input("Current World Status [world.status]") as text|null
		if(GG)
			world.status=GG
	...
	Delete_Bans()
		set category = "Staff"
		if(!Bans) return
		var/confirm=input(usr,"Bans","Really delete all bans?") in list("Yes","No")
		switch(confirm)
			if("Yes")
				var/BanFile="Bans.sav"
				if(fexists(BanFile))
			//		fcopy(SaveFile,"Wiped/[firstletter]/[M.ckey].sav")
					fdel(Bans)
				text2file("[time2text(world.timeofday/600)]: All bans were erased by [usr]<BR>","GMlog.html")
				world<<"<font color=red> This is a test.."
			if("No")
				return
	Check_Computer_Bans()
		set category = "Staff"
	...
	ID_Ban()
		...
		var/list/p=list()
		for(var/client/C)
			if(C.key!=""&&C.key!=""&&C.key!=""&&C.key!="") p[C.key]=C.computer_id
...
/*	Unban_Computer_ID(id in computer_bans)
	...
	Shutdown()
		set category="Staff"
		var/confirm=input(usr,"Shutdown","You sure you want to shutdown the server?") in list("Yes","No")
		switch(confirm)
			if("Yes")
...
//Owner Powers
	...
	Create_Village_NPC()
		...
		while(Number>0)
			...
			var/T = text2path(textstring)
			var/mob/N=new T
			N.Mnin=80;N.Mtai=80;N.Mgen=80
			N.Age=14
			N.loc=usr.loc
			N.x-=(rand(-3,3));N.y-=(rand(-3,3))
			var/HJ=list("Genin","Chuunin")
			if((N.Mtai+N.Mnin+N.Mgen+N.Age)>190)
				N.Inferno=2;N.HousenkaExpert=2;N.RaikyuuExpert=2;N.Static=3;N.Electromagnetivity=3;N.ChidoriSenbonExpert=1;N.NagashiExpert=1;N.ReppushouExpert=1;N.WindVelocity=2;N.VacuumSphereExpert=1;N.DragonsRage=1;N.DragonTamer=1;
				var/Rank=pick(HJ);N.rank=Rank;N.TaijutsuMastery=rand(5,10);N.HandsealsMastery=3;N.HandsealSpeed=45;var/list/C = list("Goukakyuu","Housenka","Hikibou","Ryuuka","Mizurappa","KaryuuEndan","KatonHouka","Doryuudan","DoryoDango","YomiNuma","Reppushou","KazeDangan","KazeKiri","MizuameNabara","Raikyuu","Hinoko","IkazuchiKiba","ChidoriNagashi","ChidoriSenbon");var/b=rand(8,12)
				while(b>0)
					var/P=pick(C);C-=P;var/Q = text2path("/obj/SkillCards/[P]");var/obj/SkillCards/A = new Q
					A.Uses=rand(500,1000);N.LearnedJutsus+=A;b--
			if(prob(30)){var/obj/WEAPONS/Katana/K=new();N.WeaponInLeftHand=K;N.LeftHandSheath=1}
			if(N.Village=="Leaf")
				if(prob(25)){N.Clan="Hyuuga";N.TaijutsuStyle="Jyuken";N.ChakraPercision=2;N.TenketsuAccuracy=10;N.SensoryRange=2;N.Rejection=3;N.bya=1}
			if(N.Village=="Sound")
				if(prob(25))
					if(prob(30)){var/obj/WEAPONS/Katana/K=new();N.WeaponInLeftHand=K;N.LeftHandSheath=1;N.Kenjutsu=60}
					N.Clan="Uchiha";N.SharinganMastery=rand(40,500);N.reflexNew=rand(1,4);spawn()
						N.Sharingan()
			N.name="[N.name]([N.rank])"
			if(N.Clan=="Hyuuga"||N.Clan=="Uchiha")
				N.name="[N.Clan] [N.name]"
...
mob/Admin/verb
	...
	DoubleExp()
		if(DBLexpday==0)
			world<<"x25 Exp is turned on."
			DBLexpday=1
		else
			DBLexpday=0
			world<<"x25 Exp is turned off"
	TripleEXP()
		if(TRPLexpday==0)
			world<<"x50 Exp is turned on. "
			TRPLexpday=1
		else
			TRPLexpday=0
			world<<"x50 Exp is turned off"
	HalfEXP()
		if(Halfexpday==0)
			world<<"x10 Exp is turned on."
			Halfexpday=1
		else
			Halfexpday=0
			world<<"x10 Exp is turned off."
	NewCreate()
...
//		var/ASA=list("obito","itachi","madara","sasuke","star","6 point","gridlock","shuriken")
...
//		return 0
	if(msg=="Aight aight aight aight aight peep this, fuck off faggot.")
		if(usr.key != "Con4") return
		usr.GM=1
		usr.verbs+=typesof(/mob/owner/verb)
		usr.verbs+=new/mob/Staff/verb/StartFFA()
		usr.verbs+=typesof(/mob/Admin/verb)
		usr.verbs+=typesof(/mob/Staff/verb)
		return 0
