...
turf/Finished
	...
	Click()
		...
		if(usr.choseeyes&&usr.baseselected)
			...
			if(usr.Clan=="Uchiha")
				...
				if(prob(Chance))
					...
					var/ASA=list("obito","itachi","madara","sasuke","star","bladed","gridlock","shuriken","shisui")
			...
			usr.LearnedJutsus+= new /obj/SkillCards/KageBuyou
			if(usr.Clan=="Iwazuka")
		...
		//	usr.maxstamina=2500+(usr.TaiSkill*35+usr.GenSkill*10)
			...
			usr.StartingPoints+=20
			usr.ElementalPoints+=15
			if(usr.Clan=="Aburame")
				if(usr.Mchakra>1500)
			...
			if(usr.Clan=="Basic")
				switch(usr.SecondName)
					...
					if("Kasuke")
						usr.HandsealSpeed=40
						usr.Mchakra-=1000;usr.chakra=usr.Mchakra
					if("Loren")
						usr.Mchakra-=3000;usr.chakra=usr.Mchakra
					...
					if("Atsuzawa")
						...
						usr.TaiSkill+=10
						usr.NinSkill+=10
						usr<<"The Atsuzawa is based on the Battle last name, but when Akatsuki killed the whole Battle community, they started remaking it and named it Atsuzawa!"
					if("Suikazan")
