...
mob/var/AgeEXPMax=52400
////////////////////////////////////////////
//Stats
mob/var/CBleeding=0
mob/var/ImportPermission=1
...
//Aburame
	...
	BugManipulation=20//MaxLevel=30, Increasing this increases the amount of bugs created within an Aburame's body.
	BugKeeper=0
	BugCapacity=0//MaxLevel=30,
	Feast=0
	KekkaiHealth=0
	KekkaiCap=0
//Akimichi
...
//Fuuma
	...
	Reaction=0
//Hatake
	WhiteChakra=100//MaxLevel=100, Increasing this increases the user's ability all together. For every increase of this, the user's chakra increases and also their ability with the White Tanto sword allows a damage increase.
	CanGetOneEyeShar=0
...
//Hoshigaki Variables 1.1
	samehadeChakaraDrain =0
	samehadeThrowingStrength =0
	samehadeStrength = 0
	samehadeQuest = 1
	samehadeUnleash = 0
	samehadeChakara = 0
...
//Uchiha
	...
	TomoeThree=0//New Passive for Unlocking Three tomoe Sharingan. Little more, 'RP eccentric'
	SharinganMastery=0
//Basic Passives
...
mob/var
	...
	ThrowingStrength=0//MaxLevel=5, Increasing this increases the speed of weapons,
...
mob/var
	...
	Focus=0 //MaxLevel=5, Increases chance of getting a critical hit on a target for each strike.
...
mob
	...
	//		src.health=0
		...
		//		src.Running=1
			...
			stat("Season: [Season]")
			stat("-------------------{[usr]}-------------------")
			/*var/WhatMonthItIs="January"
			if(src.MonthBorn==2) WhatMonthItIs="February"
			if(src.MonthBorn==3) WhatMonthItIs="March"
			if(src.MonthBorn==4) WhatMonthItIs="April"
			if(src.MonthBorn==5) WhatMonthItIs="May"
			if(src.MonthBorn==6) WhatMonthItIs="June"
			if(src.MonthBorn==7) WhatMonthItIs="July"
			if(src.MonthBorn==8) WhatMonthItIs="August"
			if(src.MonthBorn==9) WhatMonthItIs="September"
			if(src.MonthBorn==10) WhatMonthItIs="October"
			if(src.MonthBorn==11) WhatMonthItIs="November"
			if(src.MonthBorn==12) WhatMonthItIs="December"

			stat("Birthday: [WhatMonthItIs] [DateBorn], [YearBorn]")
			var/Age=(Year-YearBorn)
			if(MonthBorn<Month)
				Age-=1
			else if(MonthBorn==Month)
				if(DateBorn<Day)
					Age-=1*/
			stat("Age: [Age]")
			stat("Growth: [(AgeEXP/AgeEXPMax)*100]%")
			...
			if(src.Clan=="Hyuuga"&&src.Age>=15) stat("Branch: [src.HyuugaClanBranch]");
			stat("Vitality: [(health/maxhealth)*100]%:[maxhealth]")
			stat("Stamina: [(stamina/maxstamina)*100]%:[maxstamina]")
			stat("Chakra: [(chakra/Mchakra)*100]%:[Mchakra]")
	...
	//			stat("Eye Mastery: [src.ConversusinMastery]");
			...
			stat("Body Wounds: [(deathcount)*10]%")
		...
		//	stat("Control: [gg*10]%/Stat: [src.GenSkill]")
			stat("Physique Stat: ([tt*10]%)")
			stat("Capacity Stat: ([nn*10]%)")
			stat("Control Stat: ([gg*10]%)")
			...
			if(src.FireE||src.WaterE||src.LightningE||src.EarthE||src.WindE)
				...
				if(src.WindE)
					stat("Fuuton")
				if(src.Clan=="Shiroi"||src.Clan=="Kusakin"||src.Clan=="Shojin"||src.Clan2=="Shiroi"||src.Clan2=="Kusakin"||src.Clan2=="Shojin")
					if(src.WindE&&src.WaterE&&src.Clan=="Shiroi"||src.WindE&&src.WaterE&&src.Clan2=="Shiroi")
						stat("Hyouton")
					if(src.EarthE&&src.WaterE&&src.Clan=="Kusakin"||src.EarthE&&src.WaterE&&src.Clan2=="Kusakin")
						stat("Mokuton")
					if(src.EarthE&&src.Clan=="Shojin"||src.EarthE&&src.Clan2=="Shojin")
						stat("Shoton")
			stat("-------------------{Else}--------------------")
			...
			if(src.Clan=="Kamizari"||src.Clan2=="Kamizari")
				stat("Konchuu: [Konchuu]")
	...
	//			stat("Sand: [SandAmount]")
			if(src.CanNavigate)
				stat("Location:X [usr.x] Y [usr.y] Z [usr.z]")
...
mob/proc/LearnThisJutsu()
	if(src.JutsuEXPCost>=src.JutsuMEXPCost)
		...
		if(src.JutsuInLearning=="Rakanken")
			...
			src.TypeLearning=""
			return
		if(src.JutsuInLearning=="NinTai")
			src.TaijutsuStyle="NinTai"
			src.JutsuDelay=src.JutsuMEXPCost
			src.JutsuInLearning=""
			src.JutsuEXPCost=0
			src.JutsuMEXPCost=0;src.exp=0
		...
		if(src.JutsuInLearning=="View Gate")
			...
			src.TypeLearning=""
			return
		if(src.JutsuInLearning=="Wonder Gate")
			src.WonderGate=1
			src.JutsuDelay=src.JutsuMEXPCost
			src.JutsuInLearning=""
			src.JutsuEXPCost=0
			...
			src.TypeLearning=""
			return
		if(src.JutsuInLearning=="Death Gate")
			src.DeathGate=1
			src.JutsuDelay=src.JutsuMEXPCost
			src.JutsuInLearning=""
			src.JutsuEXPCost=0
...
turf
	DblClick()
		...
		if(usr.Mogu)
			...
			usr.CreateCrator()
		if(usr.Clan=="Akimichi"&&usr.Pill=="Red"||usr.Clan2=="Akimichi"&&usr.Pill=="Red")
		...
		//		return
			...
			if(src&&!src.density&&usr.cantele&&USER.FrozenBind==""&&!USER.Frozen&&USER.Stun<1&&USER)
				if(src.z!=USER.z)
					return
				flick("zan",USER)
				if(USER.Clan=="Sabaku"||USER.Clan2=="Sabaku")
				...
				if(USER.Clan=="Shojin"||USER.Clan2=="Shojin")
					for(var/mob/Jutsu/Elemental/Shoton/CrystalMirror/A in USER.JutsuList)
						if(A.Owner==USER)
							A.loc=USER.loc;A.dir=USER.dir
				if(usr.SelfHeal)
					usr.SelfHeal=0
		...
		if(usr.MindGenjutsuReady&&!(ismob(src)))
			if(("shift" in list_params))
				...
				if(usr.MindGenjutsuMode=="Village Say")
					...
					if(T.Village=="Cloud")
						T<<"<font color = yellow><font size= 2>(Vsay)([C])<font size= 1> - <font color=#fff600><b>[B]: [A]"
						usr<<"<font color = yellow><font size= 2>(Vsay)([C])<font size= 1> - <font color=#fff600><b>[B]: [A]"
					if(T.Village=="Snow")
						T<<"<font color = white><font size= 2>(Vsay)([C])<font size= 1> - <font color=#e3e3e3><b>[B]: [A]"
						usr<<"<font color = white><font size= 2>(Vsay)([C])<font size= 1> - <font color=#e3e3e3><b>[B]: [A]"
					if(T.Village=="Mist")
						T<<"<font color = blue><font size= 2>(Vsay)([C])<font size= 1> - <font color=#64b9ef><b>[B]: [A]"
						usr<<"<font color = blue><font size= 2>(Vsay)([C])<font size= 1> - <font color=#64b9ef><b>[B]: [A]"
	...
	MouseDown(location,control,params)
			...
			if(usr.MindGenjutsuReady&&usr!=src)
				...
				if(usr.MindGenjutsuMode=="Village Say")
					...
					if(T.Village=="Rock")
						...
						usr<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#A0522D><b>[B]: [A]"
					if(T.Village=="Mist")
						T<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#1aa9de><b>[B]: [A]"
						usr<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#1aa9de><b>[B]: [A]"
					if(T.Village=="Cloud")
						T<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#e8e8e8><b>[B]: [A]"
						usr<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#e8e8e8><b>[B]: [A]"
					if(T.Village=="Snow")
						T<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#abe8ff><b>[B]: [A]"
						usr<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#abe8ff><b>[B]: [A]"
					if(T.Village=="Rain")
						T<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#1B82E6><b>[B]: [A]"
						usr<<"<font color = green><font size= 2>(Vsay)([C])<font size= 1> - <font color=#1B82E6><b>[B]: [A]"
					if(T.Village=="Sound")
		...
		if(!src.client)
			...
			if(src.name in usr.namesknown)
				...
				if(usr.ARank=="Healer")
					if(src.deathcount<=2.5)
						usr<<"[src] still has the strength to fight."
					else if(src.deathcount>2.5&&src.deathcount<=5)
						usr<<"[src] is badly damaged, but seems to be capable to go on."
					else if(src.deathcount>5)
						usr<<"[src] looks to be on the brink of death."
			else
				usr<<"<b>You target the [src.Village] Ninja."
				if(usr.XiveresMember||usr.XiveresLeader)
					if(src.deathcount<=2.5)
				...
				if(usr.Healer)
					if(src.deathcount<=1)
						usr<<"[src] doesn't seem to be damaged at all."
					else if(src.deathcount>1&&src.deathcount<=2.5)
						usr<<"[src] still has the strength to fight."
					else if(src.deathcount>2.5&&src.deathcount<=5)
						usr<<"[src] is badly damaged, but seems to be capable to go on."
					else if(src.deathcount>5)
...
mob/owner/verb/MakeYonkami(mob/M in world)
		M.Village = "Yonkami"
		M.Yonkami = 1


mob/owner/verb/MakeXiveresLeader(mob/M in world)
	set category="Staff"
	if(M.rank=="Jounin"&&M.XiveresLeader==0)
		M.Village="Xiveres"
		M.XiveresLeader=1
		M.XiveresMember=1
		...
		A.Move(M)
		var/obj/Clothes/XiveresLeaderMask/A3=new()
		A3.Move(M)
	else
		usr<<"Cannot be done, Hes either weaker than Jounin or already in Xiveres."
...
mob/var
	...
	akatsukimember=0
	akatsukileader=0
	sevenswordsmenleader=0
	sevenswordsmenmember=0
...
mob/owner/verb/MakeAkatLeader(mob/M in world)
	set category="Staff"
	if(M.rank=="Jounin"&&M.akatsukimember==0)
		M.Village="Akatsuki"
		var/obj/Clothes/AKAH/H = new()
		var/obj/Clothes/AKAS/S = new()
		var/obj/BijuuSealingRing/Ichibi_Ring/I = new()
		var/obj/BijuuSealingRing/Nibi_Ring/N = new()
		var/obj/BijuuSealingRing/Sanbi_Ring/D = new()
		var/obj/BijuuSealingRing/Yonbi_Ring/Y = new()
		var/obj/BijuuSealingRing/Gobi_Ring/G = new()
		var/obj/BijuuSealingRing/Rokubi_Ring/R = new()
		var/obj/BijuuSealingRing/Shichibi_Ring/F = new()
		var/obj/BijuuSealingRing/Hachibi_Ring/J = new()
		var/obj/BijuuSealingRing/Kyuubi_Ring/L = new()
		H.loc=M
		S.loc=M
		I.loc=M;N.loc=M;D.loc=M;Y.loc=M;G.loc=M;R.loc=M;F.loc=M;J.loc=M;L.loc=M
		M.akatsukileader=1
		M.akatsukimember=1
		M.verbs += typesof(/mob/Akat/verb)
		M.verbs += typesof(/mob/AkatMember/verb)
	else
		usr<<"Cannot be done, Hes either weaker than Jounin or already in Akatsuki."
mob/Akat/verb/InviteA(mob/M in world)
	set category="Organization"
	set name="Organization Invite"
	switch(input(M,"Would you like to join Akatsuki Orginization?","Join Akatsuki Orginization?","") in list ("Yes","No"))
		if("Yes")
			src.Village="Akatsuki"
			if(!M.akatsukimember&&!M.akatsukileader)
				...
				usr<<"Cannot be done, they're already in the Akatsuki."
		if("No")
			usr<<"[M] denied your invite."
mob/Akat/verb/BootA()
	set category="Organization"
	...
	for(var/mob/M in world)
		...
		if(M.key != usr.key)
			if(M.akatsukimember)
				Menu.Add(M)
	var/mob/M = input("Boot Who?","Akatsuki Boot") as null | anything in Menu
	if(!M)return
	if(istype(M,/mob))
		for(var/obj/Clothes/AKAH/H in M.contents)
			del(H)
		for(var/obj/Clothes/AKAS/S in M.contents)
			del(S)
		M.akatsukimember=0
	else
		usr<<"Cannot be done, Hes either the leader or not even in Akatsuki"
mob/AkatMember/verb/TalkWithMembers(msg as text)
	set category="Organization"
	...
	for(var/mob/M in world)
		if(M.akatsukimember||M.akatsukileader)
			M<<"<font size=2><font face=trebuchet MS><font color=#CCFFFF>(Akatsuki Say)-<font size=1>[usr.oname]<font color=#DD0000>: [msg]</font>"
mob/AkatMember/verb/AkatsukiWho()
	set category="Organization"
	set name="Akatsuki Who"
	usr<<"<font color=red>Online Akatsuki Members -"
	for(var/mob/M in world)
		if(M.akatsukimember&&M.client){usr<<"<font color=green>[M.oname]"}
...
mob/Yonk
	verb/YonkInvite(mob/M in world)
		set category = "Yonkami"
		set name = "Invite to Yonkami"
		switch(input(M,"Would like to join the Yonkami?","Join?","") in list ("Yes","No"))
			if("Yes")
				M.Village = "Yonkami"
				M.Yonkami = 1
mob/Xiv
	...
	verb/AssignARank(mob/M in world)
		...
		var/Number=input(usr,"What rank do you want to assign [M]?",) as num
		M.ARank=Number
		for(var/mob/XiveresMember in world)
			if(XiveresMember.XiveresMember)
				XiveresMember<<"<font size=2><font face=trebuchet MS><font color=red>[M] is now Rank [Number] in Xiveres."
mob/Xiv
...
mob/owner/verb/MakeSevenSkiesLeader(mob/M in world)
	set category="Staff"
	if(M.rank=="Jounin"&&M.akatsukimember==0)
		M.SevenSkiesLeader=1
...
mob/Amekoutei/verb/Make7SMLeader(mob/M in world)
	set category="Kage"
	if(M.rank=="Jounin"&&M.sevenswordsmenmember==0)
		var/obj/WEAPONS/Hiramekarei/Y = new()
		var/obj/WEAPONS/OriginalSamehade/E = new()
		var/obj/WEAPONS/OriginalKubikiriHoucho/R = new()
		var/obj/WEAPONS/Nuibari/P = new()
		var/obj/WEAPONS/Kiba/A = new()
		var/obj/WEAPONS/Shibuki/D = new()
		A.loc=M
		D.loc=M
		P.loc=M
		E.loc=M
		R.loc=M
		Y.loc=M
		M.sevenswordsmenleader=1
		M.sevenswordsmenmember=1
		M.verbs += typesof(/mob/sevenswordsmen/verb)
		M.verbs += typesof(/mob/sevenswordsmenmember/verb)
mob/sevenswordsmen/verb/InviteA(mob/M in world)
	set category="Seven Swordsmen"
	set name="7SM Invite"
	switch(input(M,"Would you like to join Seven Swordsmen?","Join Seven Swordsmen?","") in list ("Yes","No"))
		if("Yes")
			if(!M.sevenswordsmenmember&&!M.sevenswordsmenleader)
				M.sevenswordsmenmember=1
			else
				usr<<"Cannot be done, they're already in the sevenswordsmen."
		if("No")
			usr<<"[M] denied your invite."
mob/sevenswordsmen/verb/BootA()
	set category="Seven Swordsmen"
	set name="Organization Boot"
	var/list/Menu = list()
	for(var/mob/M in world)
		if(!M.client) continue
		if(M.key != usr.key)
			if(M.sevenswordsmenmember)
				Menu.Add(M)
	var/mob/M = input("Boot Who?","7SM Boot") as null | anything in Menu
	if(!M)return
	if(istype(M,/mob))
		M.sevenswordsmenmember=0
	else
		usr<<"Cannot be done, Hes either the leader or not even in 7SM"
mob/sevenswordsmenmember/verb/TalkWithMembers(msg as text)
	set category="Seven Swordsmen"
	set name="7SM Say"
	set desc = "Say something to every high official of the village."
	...
	for(var/mob/M in world)
		if(M.sevenswordsmenmember||M.sevenswordsmenleader)
			M<<"<font size=2><font face=trebuchet MS><font color=#CCFFFF>(7SM Say)-<font size=1>[usr.oname]<font color=#DD0000>: [msg]</font>"
mob/sevenswordsmenmember/verb/SevenSwordsmenWho()
	set category="Seven Swordsmen"
	set name="7SM Who"
	usr<<"<font color=red>Online 7SM Members -"
	for(var/mob/M in world)
		if(M.sevenswordsmenmember&&M.client){usr<<"<font color=green>[M.oname]"}
