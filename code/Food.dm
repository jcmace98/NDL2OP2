...
obj/MindPill
	name = "Mind Awaken Pill"
	icon = 'Food.dmi'
	icon_state="6"
	verb
		Eat()
			usr<<"You eat the [src]!"
			if(usr.CursedSealPool<1000)
				usr.CursedSealPool=1000
			usr.calories+=500
			usr.hunger=0
			usr.thirst=0
			del(src)
		...
		Get()
			...
			usr<<"You picked up a [src]"
////////////////////////////////////////
mob/proc/CreateMindPill()
	var/CreatingPill=0
	if(CreatingPill== 1)
		src<<"You are already creating a Mind Awakening Pill"
		return
	CreatingPill=1
	src.Frozen = 1
	src<<"You begin cultivating the pill inside your body.."
	src.chakra -= 1000
	sleep(100)
	src<<"You release your chakra into your hand, cultivating the pill in the palm of your hand."
	sleep(200)
	src<<"You begin to feel an immense, sharp pain in your body."
	src.stamina -= 500
	sleep(100)
	src<<"The pill fully forms in the middle of your palm as it gives off an ominous chakra."
	CreatingPill=0
	src.Frozen = 0
	src.maxstamina-=1000
	src.Mchakra-=750
	src.DeclineAge-=10
	src<<"Creating the pill has cost you some of your vitals."
	var/obj/MindPill/A=new();A.loc=src
mob/proc/GrantCS(var/mob/M in get_step(src,src.dir))
	if(usr.CurseGrant>7)
		return;usr<<"You can't use this anymore.."
	if(!(locate(/obj/SkillCards/CursedSealActivation) in M.LearnedJutsus))
		switch(input(usr,"Do you wish to grant the power of the Curse Mark?") in list("Yes","No"))
			if("Yes")
...
obj/Drinks
	...
	verb
		...
		Drink()
			...
			var/EatingTime=src.FoodTimeInSeconds*10
			sleep(EatingTime)
			if(usr.thirst==0&&!usr.Clan=="Ketsueki"||usr.thirst==0&&!usr.Clan2=="Ketsueki")
				usr<<"You feel like you're about to overflow!";usr.health=0;usr.Frozen=0;return
			usr.Frozen=0
			usr<<"You finished drinking."
			if(usr.Clan=="Satou"&&src.name=="Sake"||usr.Clan2=="Satou"&&src.name=="Sake")
				if(prob(40))
