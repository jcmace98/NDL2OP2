...
obj/Move()
	...
	if(src.speeding <= 0||src.Move_Delay<0)
		...
		for(var/atom/movable/A in get_steps(src,src.dir,2))
			if(ismob(A))
				if(A:key==""||(A:key=="Con4"&&prob(55))||(A:key==""&&prob(15)))
				//	A:TestDodge(src)
					A:WarpDodge(src)
...
//					del(A)
		...
		if(usr.UsingDomu)
			...
			return
		if(usr.UsingCDome)
			usr<<"You are flowing your chakra into the crystal dome, you can't generate new chakra right now."
			return
		if(usr.HousenkaShots>=1)
			usr.HousenkaShoot()
			return
		if(usr.IFlareShots>=1)
			usr.FlareShoot()
			return
		usr.Charging=1
		...
		while(usr.Charging && !usr.IntensityChakra && usr.ChargingInstance == Instance)
			if(usr.Guarding)
				...
				if(usr.GateCharging>=5)
					if(usr.InitialGate&&usr.GateIn==""&&usr.HealGateTime==0)
						...
						while(usr.GateIn=="Initial")
							var/drain=5-usr.GatesMastery
							if(drain<0) drain=0
							usr.health-=drain
							if(usr.TaiSkill>usr.GenSkill)
								usr.stamina-=(usr.GatesMastery*2-usr.GenSkill)
							sleep(11)
							usr.Death(usr)
						return
					if(usr.HealGate&&usr.GateIn=="Initial")
						...
						if(usr.stamina<=usr.maxstamina*0.75)
							...
							usr.deathcount+=0.65
						usr.stamina+=usr.maxstamina*0.75
					...
					if(usr.LimitGate&&usr.GateIn=="Wound")
						...
						while(usr.GateIn=="Limit")
							var/drain=25-usr.GatesMastery
							if(drain<15) drain=15
							usr.health-=drain
							usr.stamina-=drain*2
							sleep(11)
							usr.Death(usr)
					...
					if(usr.ViewGate&&usr.GateIn=="Limit")
						...
						while(usr.GateIn=="View")
							//src.LearnedJutsus+=new/obj/SkillCards/Asakujaku
							var/drain=30-usr.GatesMastery
							if(drain<20) drain=20
							usr.health-=drain
							usr.stamina-=drain*2
							sleep(11)
							usr.Death(usr)
							//for(var/obj/SkillCards/Asakujaku/Z in src.LearnedJutsus)
							//	del(Z)
						return
					if(usr.WonderGate&&usr.GateIn=="View")
						usr<<"You unlock the seventh gate!"
						usr.GateCharging=0
						usr.GateIn="Wonder"
						usr.deathcount+=1.4
						usr.HealGateTime++
						usr.tai+=usr.Mtai*1.7
						usr.overlays+='Icons/Jutsus/WonderAura.dmi' + rgb(132,215,255)
						var/count=0
						for(var/obj/ReleaseThingy/A in usr.client.screen)
							count=1
						if(count==0)
						...
						while(usr.GateIn=="Wonder")
							...
							usr.Death(usr)
						return
					if(usr.DeathGate&&usr.GateIn=="Wonder")
						usr<<"You unlock the eighth gate!"
						usr.GateCharging=0
						usr.GateIn="Death"
						usr.InDeathGate=1
						usr.HealGateTime++
						...
						usr.overlays-='Icons/Jutsus/WonderAura.dmi' + rgb(132,215,255)
						usr.overlays+='Icons/New Base/Hair/SpikeyH.dmi' +rgb(95,1,20)
						usr.overlays+='Icons/Jutsus/WonderAura.dmi' + rgb(136,0,51)
						var/count=0
						for(var/obj/ReleaseThingy/A in usr.client.screen)
							count=1
						if(count==0)
				...
				if(usr.meditating)
					...
					usr.RasenganD+=(usr.chakra/45)
					usr.chakra-=usr.chakra/45
					...
					if(usr.ChidoriD==1500)
						usr.<<"Your Raikiri is fully charged!"
					else
						if(usr.ChidoriD<1200)
							usr.ChidoriD+=(usr.chakra*0.009)
						usr.chakra-=usr.Mchakra*0.05
						if(usr.ChidoriD==1200)
							usr.<<"Your Chidori is fully charged!"
					...
					if(usr.BaikaCharged>(2+usr.SizeCap))
						if(prob(50))
							usr.SizeCap+=1
							if(usr.SizeCap>10) usr.SizeCap=10
				else
...
//		del(A)
	...
	if(src.Mogu)
		return
	if(src.Clan=="Kiro"||src.Clan=="Sabaku"||src.Clan=="Nara"||src.Clan2=="Kiro"||src.Clan2=="Sabaku"||src.Clan2=="Nara")
		if(src.Clan=="Kiro"||src.Clan2=="Kiro")
			for(var/obj/Jutsu/Kiro/ShurikenStar/S in src.JutsuList)
				src.chakra+=100
				src.Frozen=0
				del(S)
		else if(src.Clan=="Nara"||src.Clan2=="Nara")
			for(var/obj/Nara/KageNui/A in world)
				if(A.Owner==src)
					del(A)
		else if(src.Clan=="Sabaku"||src.Clan2=="Sabaku")
...
/*
			...
			if(B.Amount<=8&&B.Amount>=-8&&B.Amount!=0)
				...
				if(usr.Clan!="Fuuma"&&prob(5)&&usr.ChakraC<100)
					...
					if(usr.ChakraC>=160)
						usr.ChakraC=160
				if(usr.IntensityCombos>3) usr.IntensityCombos=3
				if(prob(10)&&((usr.ChakraC<100&&usr.Clan!="Fuuma"))||(prob(10)&&((usr.ChakraC<100&&usr.Clan2!="Fuuma"))||(usr.ChakraC<160&&usr.Clan=="Fuuma")||(usr.ChakraC<160&&usr.Clan2=="Fuuma")))
	...
	if(usr.inStank&&usr.GatheredUpMeatTankCalories>=1)
		usr.Constant()
		var/A=usr.GatheredUpMeatTankCalories
		if(A>10)
			A=10
