	...
	//	src.CopyMode=0;
		...
		if(!src.Indigo)
			...
			if(src.IndigoMastery<130&&src.IndigoMastery>=60)
				...
				view()<<"[src]'s eyes shine the darkest shade of indigo, revealing a dark indigo that resembles the most evil form of darkness and despair.";
				src.HandsealSpeed=40
				src.IndigoBlock=1
				I.icon_state="Tomoe1"
		...
		while(i>0&&get_dist(M,src)<=1&&src.DotonSpear&&!M.knockedout)
			...
			if(M&&(M.knockedout!=1)&&(M.sphere!=1)&&(M.Kaiten!=1))
				...
				if(M.JC>0)
					...
					if(M.Guarding||M.NenkinOn||M.CArmorOn)
						viewers()<<sound('SFX/Slice.wav',0)
						if(!M.NenkinOn||!M.CArmorOn)
							if(M.health>0)
