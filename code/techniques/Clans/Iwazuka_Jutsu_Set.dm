...
//			HaveClay=1;
	...
	CreateFlyingBird()
		...
		if(!src.knockedout)
			...
			src.controlled=A;src.client.perspective=EYE_PERSPECTIVE|EDGE_PERSPECTIVE;src.client.eye=A;src.OnBird=1
			Create_Shadow(src)
			Flight(src)
			while(A)
				src.icon_state=""
				src.layer=A.layer;src.OnBird=1
	...
	CreateExposiveSpike()
		...
		if(!HaveClay)
			src<<"You've ran out of Clay!";return
		src.chakra-=4000;src<<"Click to have expolsive spike rise!"
		spawn(200)
