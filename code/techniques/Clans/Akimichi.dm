			...
			if(!src.BaikaCharged||src.calories<=0)
				...
				for(var/obj/SkillCards/Nikudan/A in src.LearnedJutsus)
					...
					spawn(250-src.NinjutsuMastery)
						A.overlays-='Icons/Jutsus/Skillcards2.dmi'
				for(var/obj/SkillCards/NikudanHari/B in src.LearnedJutsus)
					B.Delay=1;spawn(400-src.NinjutsuMastery)
						B.Delay=0
					B.overlays+='Icons/Jutsus/Skillcards2.dmi'
					spawn(400-src.NinjutsuMastery)
						B.overlays-='Icons/Jutsus/Skillcards2.dmi'
				src.intank=0
				src.Endurance-=500
			sleep(11)
///////////////////////////////////////////////////////
//Ninkudan Hari Sensha
//////////////////////////
mob/var/tmp/inStank=0
mob/proc/NikudanHari()
	if(!src.BaikaCharged>=1)
		src<<output("You need to be in Baika No Jutsu first!","Attack");return
	else
		var/StaminaLoss=300
		src.stamina-=StaminaLoss
		src.overlays-='Icons/Jutsus/AkimichiTechniques2.dmi';src.overlays+='Icons/Jutsus/AkimichiTechniques2.dmi'
		src.inStank=1;src.firing=1;src.Constant()
		src.Endurance+=500
		while(src.inStank)
			src.calories-=10
			if(!src.BaikaCharged||src.calories<=0)
				src.Normal();walk(usr,0);src.overlays-='Icons/Jutsus/AkimichiTechniques2.dmi';src.firing=0
				for(var/obj/SkillCards/Nikudan/A in src.LearnedJutsus)
					A.Delay=1;spawn(250-src.NinjutsuMastery)
						A.Delay=0
					A.overlays+='Icons/Jutsus/Skillcards2.dmi'
...
//ButterflyMode/////////////////////
///////////////////////////////////
mob/var/tmp/Butterfly=0
mob/proc/ButterflyRelease()
	if(src.knockedout)
		return
	if(src.Butterfly)
		src.Butterfly=0
		src<<"You stop the rapid conversion of calories into chakra!"
		src.underlays-='Icons/Jutsus/ButterflyModeUnderlay.dmi'
		src.overlays-='Icons/Jutsus/ButterflyModeOverlay.dmi'
		src.tai=src.Mtai
	else
		src<<"You begin to rapidly convert your calories into chakra, greating increasing your physical abilities!"
		src.underlays+='Icons/Jutsus/ButterflyModeUnderlay.dmi'
		src.overlays+='Icons/Jutsus/ButterflyModeOverlay.dmi'
		src.Butterfly=1
		while(src.Butterfly)
			src.tai=src.Mtai*2.5
			src.calories-=rand(9,18)
			src.chakra-=4
			if(src.calories<5000)
				src<<"You're unable to sustain this form with your current body calories."
				src.ButterflyRelease()
			sleep(1)
		src.underlays-='Icons/Jutsus/ButterflyModeUnderlay.dmi'
		src.overlays-='Icons/Jutsus/ButterflyModeOverlay.dmi'
