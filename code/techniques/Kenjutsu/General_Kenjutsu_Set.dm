mob/proc
	SwiftSlashTechnique(Hand)
		...
		for(var/mob/M in oview(1,src))
			...
			if(SizeRating==3&&src.Clan!="Kaguya")
				src<<"You can't do a rapid strike with a sword like this.";src.firing=0;return
	...
	RapidStrike(Hand)
		...
		for(var/mob/M in get_step(src,src.dir))
			...
			var/SizeRating=CCA.SizeRating
			if(SizeRating==3&&src.Clan!="Kaguya")
				src<<"You can't do a rapid strike with a sword like this.";src.Frozen=0;return
			var/Type="Normal"
	...
	Moonsplitter()
		if(AbleToMoonsplitter==0)
			src<<"You weren't quick enough to use this!"
			return
		if(src.firing)
			return
		if(src.target)
		...
		//			else
		//				src.SliceHit(M,Damage,Type,Power,SizeRating)
				src<<"You swing your sword in the shape of a crescent!"
				M<<"[src] swings his sword in the shape of a crescent!"
				M.health-=Damage4
				M.StunAdd(1)
			sleep(1)
		src.Frozen=0;return