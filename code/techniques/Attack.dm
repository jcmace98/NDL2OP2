...
mob/GainedAfterLogIn/verb
		...
		if(usr.GenerativeScrollOn)
			...
			//		del(A)
				...
				if(usr.GenerativeScrollOn==4)
					if(prob(5))
						usr.DotonKnowledge+=5
				if(usr.GenerativeScrollOn==5)
					if(prob(5))
						usr.SuitonKnowledge+=5
				if(usr.GenerativeScrollOn==6)
					if(prob(5))
						usr.RaitonKnowledge+=5
			if(usr.PressAButton=="Z")
				usr.CanUseScroll=0
				spawn(150)
					usr.CanUseScroll=1
	...
	//				del(A)
				...
				usr.Frozen=0
		/*if(usr.Clan=="Sabaku"||usr.Clan2=="Sabaku")
			if((locate(/obj/SkillCards/Mangekyo) in usr.LearnedJutsus))
				if(usr.sphere)
...
//				usr.Particle_Release()
			...
			while(usr.Guarding)
				...
				while(usr.Charging)
					sleep(1)
					if(usr.HaveBoStaffEquipped==1||usr.RinneganOn)
						sleep(5)
						usr.chakra-=50
						spawn(1) for(var/obj/Jutsu/Elemental/A in oview(3))
...
mob/GainedAfterLogIn/verb
	GRelease()
		...
		for(var/obj/Jutsu/Elemental/Raiton/Raikyuu/R in orange(20,usr))
			if(R.Owner==usr)
				del(R)
		for(var/obj/Jutsu/Elemental/Katon/Houka/H in orange(20,usr))
			if(H.Owner==usr)
				del(H)
		for(var/obj/Chibaku/C in orange(20,usr))
			if(C.Owner==usr)
				del(C)
				src.icon_state=""
				src.firing=0;
...
mob/GainedAfterLogIn/verb
	Jump()
		...
		if(src.Shibari||src.hyoushou||src.resting||src.caught||src.firing)
			return
		var/Destination = get_steps(src,src.dir,2);//var/CurrentLocation = H.loc
		...
		for(var/turf/T in Route)
			...
			for(var/mob/C in T)	//Escape should be impossible! >:O
				Caught.Add(C)
		jumping = !jumping
		var/jumplength = 5//how high
		var/jtimer = 1//length between each update of jump
		var/ftimer= 1//length between fall update
		density = 0
...
mob/GainedAfterLogIn/verb//LeftHand
	...
	//		world<<"[src] controlling a puppet detected in A2 verb."
		...
		if(usr.Guarding&&usr.ChargedChakraForPunch>0)
			usr.HitTheGround()
		if(usr.Guarding&&usr.Bijuu=="Juubi"&&usr.TailState>0)
			usr.Gudou()
		if(usr.Guarding&&usr.Rasenganon&&src.RasenganType==300||usr.Guarding&&usr.Rasenganon&&src.RasenganType==300.5||usr.Guarding&&usr.Rasenganon&&src.RasenganType==301.5||usr.Guarding&&usr.Rasenganon&&src.RasenganType==500)
			usr.ThrowRasenShuriken()
			usr.Rasenganon=0
			usr.overlays-='New Icons(Con4)/Rasen-Shuriken(NEW).dmi'
			view<<"[src] throws their Fuuton: RasenShuriken!"
...
mob/proc/AttackVerb2(var/Power)
	...
	if(src.KagDance=="Tessenka")
		...
		spawn(40)
			src.LeftHandDelay=0
		if(prob(5))
			src.DanceMastery+=pick(0.5,0.7,0.9,1.2,1.5)
	...
	if(src.Tsutakei&&src.GrassMastery>75)
		...
		return
	if(src.target)
		if(src.GateIn=="Wound"||src.GateIn=="Limit"||src.GateIn=="View"||src.GateIn=="Wonder"||src.InDeathGate==1||src.Venatu)
					...
					if(src.CSet=="C1")
						...
						for(var/mob/Kibaku/C1Spider/S in world)
							if(S.Owner==src)
								counter++
						if(counter>=21)
							src<<"You've have enough C1 spiders out. Explode them.";src.Frozen=0;return
						sleep(5)
					if(src.CSet=="C2")
						sleep(14)
					if(src.CSet=="C4")
...
//				TargetArea=oview(2,src)
			...
			if(src.WeaponInLeftHand=="CrystalSpear")
				var/Damage=(src.CrystalChakra/7)+2
				var/Delay=13
				flick("Attack",src)
				src.LeftHandDelay=1
				src.Deflection=1;spawn(2+src.DeflectionZ)
				...
				if(A.WhatDoesItDo=="Throw")
					if(istype(A,(/obj/WEAPONS/Kunai)))
						src.KunaiThrowing()
						if(prob(3)) src.KunaiMastery+=pick(0.5,0.7)
					...
					if(istype(A,(/obj/WEAPONS/Senbon)))
						...
						if(prob(5))
							src.SenbonMastery+=pick(0.5,0.7)
						spawn(10-src.Swift)
							src.LeftHandDelay=0
						if(A.ammount<=0)
					...
					if(istype(A,(/obj/WEAPONS/Shuriken)))
						...
						if(prob(3))
							src.ShurikenMastery+=pick(0.3,0.5)
						spawn(10-src.Swift)
							src.LeftHandDelay=0
						if(A.ammount<=0)
							src.WeaponInLeftHand=""
					...
					spawn(Delay-src.SpeedDice)
						...
						if(prob(25))
							if(src.Buffet<10&&src.Clan=="Yotsuki")
								src.Buffet+=0.5;if(src.Buffet>10) src.Buffet=10


					if(isobj(A,/obj/WEAPONS/Kunai))
						if(prob(5))
							src.KunaiMastery+=pick(0.5,0.7)
	...
	if(src.TaijutsuStyle=="Jyuken")
		if(prob(5))
			src.StanceMastery+=(rand(3,5)*0.1);if(src.StanceMastery>75) src.StanceMastery=75
	src.LeftHandDelay=1
	spawn()
				...
				if(prob(5))
					if(src.Swift>=6)
						if(src.DoubleStrike<10)
							src.DoubleStrike+=0.5;if(src.DoubleStrike>10) src.DoubleStrike=10
	if(src.Kumosenkyuu)
		if(src.HasArrow)
...
/////////////////////////////////////
		...
		if(src.KagDance=="Karamatsu")
			...
			for(var/mob/M in get_step(src,src.dir))
				...
				if(M.IronArmor)
					src<<output("You try to strike [M] but their Iron Sand stops you from attacking, wounding you back!","Attack");M<<output("You block out [src]'s attack, wounding them!","Attack");src.DamageProc(src.maxhealth*0.09,"Health",M);spawn()Blood(src.x,src.y,src.z);return
				if(M.DotonArmor)
		...
		for(var/obj/A in get_step(src,src.dir))
			...
			//				src.maxstamina++
					...
					if(prob(1))
						if(src.Rush<20)
							src.Rush+=0.5;if(src.Rush>20) src.Rush=20
...
//	world<<"[Power]."
	...
	if(src.target&&!src.UsingBow)
		if(src.GateIn=="Wound"||src.GateIn=="Limit"||src.GateIn=="View"||src.GateIn=="Wonder"||src.GateIn=="Death"||src.Venatu)
			flick("zan",src);var/mob/M=src.target
			if(M.z==src.z)
				var/Z=rand(1,4)
				if(Z==1)
...
//Iwazuka Stuff End
				...
				if(A.WhatDoesItDo=="Throw")
					if(istype(A,(/obj/WEAPONS/Bow)))
						if(src.UsingBow)
							src.AttackDelay=1
							if(prob(3))
								src.ArcherMastery+=pick(0.5,0.6,0.7,0.8)
							spawn(60-src.Swift)
					...
					if(istype(A,(/obj/WEAPONS/Shuriken)))
						...
						if(prob(5))
							src.ShurikenMastery+=pick(0.5,0.7)
						spawn(9-src.Swift)
							src.RightHandDelay=0
						if(A.ammount<=0)
							src.WeaponInRightHand=""
					...
					spawn(Delay-src.Swift)
						...
						if(prob(2))
							if(src.Swift<6)
								...
								spawn(15)
									src.overlays-=I
								src.Swift+=0.3;if(src.Swift>6) src.Swift=6
					for(var/mob/M in get_step(src,src.dir))
						if(M.Mogu)
							return
						src.DodgeCalculation(M)
	...
	if(src.TaijutsuStyle=="Jyuken")
		if(prob(5))
			src.StanceMastery+=pick(0.3,0.5,0.7,0.9,1.2,1.5);if(src.StanceMastery>75) src.StanceMastery=75
	src.RightHandDelay=1
	if(src.TimeToAttack==1)
		src.TimeToAttack=2
		var/style = "<style>BODY {margin:0;font:arial;background:black;\
...
mob/proc/JumpVerb()
	...
	if(usr.Status=="Asleep")
		return
	if(src.resting||src.firing||src.meditating||src.jumping)
		return
	if(src.FrozenBind!="")
		if(src.FrozenBind!="Spider Web"&&src.FrozenBind!="Syrup")
			return
...
mob/proc/SenbonThrowing(Hand)
	if(prob(1)&&src.ThrowingStrength<5)
		src.ThrowingStrength+=pick(0.5,0.8)
...
mob/proc/WShurikenThrow()
	if(prob(1)&&src.ThrowingStrength<5)
		src.ThrowingStrength+=pick(0.5,0.8)
		if(src.ThrowingStrength>5) src.ThrowingStrength=5
	//if(src.key=="Royed")	spawn(1) src.KageShuriken() //Testing Purposes Only.
...
//Calculations
		...
		if(Damage<10)
			Damage+=rand(4,6)
		if(Damage>300&&src.GateIn!="Wound"&&src.GateIn!="Limit"&&src.GateIn!="View"&&src.GateIn!="Wonder"&&src.InDeathGate!=1)
...
/////////////
		if(istype(M,/mob/Sand/SunaNoTate))
			...
			if(random==50)
				del(M)
		if(istype(M,/mob/Jutsu/Elemental/Shoton/CrystalMirror))
			var/random=rand(1,50)
			if(random==50)
				del(M)
		if(M.JC>0)
		...
		if(M.FrozenBind=="DoBind")
			...
			return
		if(M.FrozenBind=="CBind")
			src<<output("Your fists bounce harmlessly off the crystal...","Attack");M<<output("[src]'s fist bounce harmlessly off the crystal...","Attack")
			return
		if(M.Kaiten)
		...
		if(M.ingat)
			...
			if(src.ManBeastClaws||src.ConversusinSecondStage||(src.Bijuu=="Nibi"&&src.TailState>=1))
				...
				if(Style=="Jyuken")
					...
					if(prob(1)&&src.ChakraPercision<5)
						src.ChakraPercision+=pick(0.5,0.7);if(src.ChakraPercision>5) src.ChakraPercision=5
					if(prob(3)&&src.TenketsuAccuracy<20&&M.resting==0&&M.knockedout==0&&M.Dead==0&&M.client)
						src.TenketsuAccuracy+=pick(0.5,0.8);if(src.TenketsuAccuracy>20) src.TenketsuAccuracy=20
					Damage=Damage/10
				...
				if(src&&src.RaiArmor==2)
					if(M&&!M.RaiArmor)
						M.DamageProc(rand(75,110),"Health",src)
			...
			if(Damage>500||src.GateIn=="View"||src.GateIn=="Limit"||src.GateIn=="Wonder"||src.GateIn=="Death"||src.BubunBaika||Power>19)
				...
				spawn(JyuukenTime)
					if(Style=="Jyuken"&&!src.Clone)
						if(M&&!M.knockedout&&src.bya)
							if(prob(10+src.TenketsuAccuracy))
								if(!M.knockedout&&!M.ImmuneToDeath)
									var/ChakraBonusLoss=src.ChakraPercision*12
									if(src.ByakuganMastery>7000)
										ChakraBonusLoss*=1.3
									M.chakra-=M.Mchakra*0.025+ChakraBonusLoss
									if(src.ByakuganMastery>7000&&prob(90))
										M.ChakraPool-=rand(17,25)
								var/Damage2=M.maxhealth*pick(0.03*(src.TenketsuAccuracy/7),0.003*src.TenketsuAccuracy)//was originally divided by 10
								if(src.ByakuganMastery>7000)
									Damage2*=1.2
								M.DamageProc(Damage2,"Health",src)
								if(prob((src.TenketsuAccuracy*0.75)+3)&&M)
									M.DamageProc(M.maxhealth*0.04,"Health",src)
...
mob/proc
	Attacking15(mob/M)
		...
		if(M.Kaiten)
			...
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			if(src.GateIn=="Limit"|src.GateIn=="View"|src.GateIn=="Wonder"|src.InDeathGate==1)
				if(prob(50)&&M)
...
mob/proc
	Attacking16(mob/M)
		...
		if(M.Kaiten)
			...
			if(M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
			if(src.GateIn=="Limit"|src.GateIn=="View"|src.GateIn=="Wonder"|InDeathGate==1)
				if(prob(50)&&M)
					M.dir=src.dir;step(M,M.dir);step(M,M.dir);step(M,M.dir)
...
mob/proc
	KunaiHit(mob/M)
		...
		if(M.FrozenBind=="DoBind")
			...
			return
		if(M.FrozenBind=="CBind")
			src<<output("Your weapon bounces harmlessly off the crystal...","Attack");M<<output("[src]'s weapon bounces harmlessly off the crystal...","Attack")
			viewers()<<sound('SFX/Guard.wav',0,volume=35);return
			return
		if(M.Kaiten)
			src<<output("[M]'s rotation shot you back!","Attack");M<<output("Your rotation shot [src] back!","Attack");step(M,src.dir);sleep(1);step(M,src.dir);src.health-=rand(1,damage);M.RushLimitz=0;src.Death(M)
...
//		var/DamagePlus=rand(50,80)
		...
		if(M.CArmorOn)
			viewers()<<sound('SFX/Guard.wav',0,volume=30);return
		if(M.sphere)
			viewers()<<sound('SFX/Guard.wav',0,volume=29);return
		if(M.FrozenBind=="DoBind")
			src<<output("Your weapon bounces harmlessly off the rocks...","Attack");M<<output("[src]'s weapon bounces harmlessly off the rocks...","Attack")
	...
	StaveHit(mob/M,Damage,Type,Power,Cap)
		...
		if(M.Clan=="Yotsuki"&&prob((M.DeflectionZ+M.Buffet+M.Teamwork+M.SpeedDice)*1.5))
			if(M.LeftHandSheath||M.RightHandSheath)
				M<<output("Your weapons deflect each other!","Attack");src<<output("Your weapons deflect each other!","Attack");viewers()<<sound('SFX/SwordClash.wav',0,volume=45);M.Deflection=0;src.Deflection=0
				spawn() AttackEfx4(M.x,M.y,M.z,M.dir);AttackEfx4(src.x,src.y,src.z,src.dir)
			...
			viewers()<<sound('SFX/HitStrong.wav',0,volume=45)
			if(M.RaiArmor)
				src.StunAdd(10);src.health-=70;src.Death(M)
			var/AddOnDamage=(src.SlashStrength*10/2)
			if(src.SlashStrength>10)
				var/Additional=(src.SlashStrength-15)
				AddOnDamage=100+round(Additional*2)
			damage+=AddOnDamage
			if(prob(7.75*(src.HeavySlam+1)))
				damage+=100+(src.HeavySlam*2)
				M.Running=0
...
//		var/obj/WEAPONS/LELE = usr.WeaponInRightHand
	...
	//				M.client.screen+=new/obj/ReleaseThingyAilment/Bleed
			...
			if(prob(random))
				...
				spawn(15)
					del(I)
				src.Kenjutsu+=pick(1.7,2,2.2)
				if(src.Clan!="Yotsuki"&&src.Kenjutsu>100||src.Clan2!="Yotsuki"&&src.Kenjutsu>100)
					src.Kenjutsu=100
				else if(src.Clan=="Yotsuki"&&src.Kenjutsu>125||src.Clan2=="Yotsuki"&&src.Kenjutsu>125)
					src.Kenjutsu=125
			M.RushLimitz=0
			if(M&&M.Buffet>=1)
				M.BuffetGain+=1;if(M.BuffetGain>round(M.Buffet)) M.BuffetGain=round(M.Buffet)
...
/*	if(src.SytheThrow)
		...
		spawn(40)
			src.LeftHandDelay=0
		var/obj/Jutsu/TriSythe/SytheTrail/A=new();
		A.Owner=src;
...
mob/proc
	ChidoriA(mob/M)
		...
		if(src.Chidorion)
			if(damage>1000&&src.runningspeed<=15)
				damage=1000
		if(src.Raikirion)
			if(damage>1200)
				damage=1200
...
mob/proc
	RasenganA(mob/M)
		...
		if(src.RasenganType==0.5||src.RasenganType==1.5)
			damage=damage*2.5
		if(src.RasenganType==300||src.RasenganType==300.5||src.RasenganType==301.5||src.RasenganType==500)
			damage=damage*4
		if(src.RasenganType==400)
			damage=damage*3.75
		if(src.RasenganType==300||src.RasenganType==300.5||src.RasenganType==301.5||src.RasenganType==500)
			view() << "<i><font size = 2>[src] hit [M] with their Rasen-Shuriken for [damage] damage!</font></i>"
		else
			view() << "<i><font size = 2>[src] hit [M] with their Rasengan for [damage] damage!</font></i>"
		M.adir=src.dir
		if(src.RasenganType==0||src.RasenganType==50||src.RasenganType==400)
		...
		if (src.RasenganType==300)
			M.DamageProc(600+src.NinSkill,"Health",src)
			M.Status=""
			M.deathcount++
			M.deathcount++
			usr.health-=rand(250,650)
			usr.<<"Due to using RasenShuriken close range, your body took recoil damage."
		M.Stun=0
		spawn()
			M.DamageProc(damage/400,"Health",src)
			if(src.RasenganType==300||src.RasenganType==300.5||src.RasenganType==301.5||src.RasenganType==500)
				M.Bloody()
		spawn()
			M.DamageProc(damage/400,"Health",src)
