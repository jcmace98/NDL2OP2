...
mob/proc/RemoveUchihaAura()
	...
	for(var/client/CC in world)
		for(var/image/x in CC.images)
			spawn()
				if(x&&(x.layer==FLY_LAYER+98)&&x.loc==src)
					del(x)
				...
				spawn()
					...
					if(M.ChakraColorR==100&&M.ChakraColorG==100&&M.ChakraColorB==255)
						if(M.Clan=="Kyomou")
							...
							M.tempmix='Icons/Jutsus/ChakraAura.dmi'
							if(M.Clan=="Uchiha"&&src.UchihaMastery>99||M.Clan2=="Uchiha"&&src.UchihaMastery>99)
								if(M.mangekyou)
									M.tempmix=null
									I=null
									...
									if(M.Clan=="Uchiha"&&src.UchihaMastery>99&&src.DarkFlames||M.Clan2=="Uchiha"&&src.UchihaMastery>99&&src.DarkFlames)
						...
						I=image('Icons/Jutsus/ChakraAuraPink.dmi',M,"",FLY_LAYER+98)
					else
						if(M.Clan=="Kyomou")
	...
	if(src&&src.client&&OnOff==0)//was just onoff remove the src if needed
		if(src in OnlinePlayers)
			for(var/image/x in src.client.images)
				spawn(4)
					if(x&&(x.layer==FLY_LAYER+98||x.icon=='Icons/Jutsus/ChakraAuraBlue.dmi'||x.icon=='Icons/Jutsus/ChakraAuraRed.dmi'||x.icon=='Icons/Jutsus/ChakraAuraPink.dmi'))
						del(x)
					if(!src)
						return
...
obj/Jutsu
				...
				for(var/mob/M in oview(1,src))
					spawn()
						if(M.SensoryRange>=5&&M.ByakuganMastery>1000&&!M.Frozen&&M.FrozenBind==""&&prob(40))
							step_away(M,src)
						else if(M&&!M.UsingDomu&&!M.Kaiten&&!M.sphere&&!M.SusanooIn&&!M.UsingCDome)
	...
	SmallExplosion
		...
		//					sleep(1)
					...
					for(var/mob/M in oview(1,src))
						if(!M.UsingDomu&&!M.Kaiten&&!M.SusanooIn&&!M.UsingCDome)
				...
				for(var/mob/M in oview(1,src))
					spawn()
						if(M.SensoryRange>=5&&M.ByakuganMastery>1000&&!M.aflagged&&!M.Frozen&&M.FrozenBind==""&&prob(40))
							...
							var/mob/O=src.Owner
							var/damage=M.maxstamina*(0.02*(O.ExplosiveMastery/20))
							var/damage2=M.maxhealth*(0.009*(O.ExplosiveMastery/20))
							if(M.Guarding)
								damage/=10
								damage2/=2
							if(M.Mogu)
	...
	C3Explosion
		...
		New()
			...
			spawn()
				...
				for(var/mob/M in oview(1,src))
					spawn()
						if(M&&!M.UsingDomu&&!M.Kaiten&&!M.sphere&&!M.SusanooIn&&!M.UsingCDome)
...
//		return//Just added 12/11/2012
	...
	if(WhereItHits=="Health")
		...
		if(B&&src.BloodLink==1&&B in view(10,src))
			...
			src.health-=Damage
			if(JutsuName!=null)
				src<<output("<font color=[MessageColor]>You were hit by the [JutsuName] for [Damage] vitality damage! (Protected from [100-(round(Damage/StartDamage)*10)]%)</font>","Attack")
				...
				if(MessageColor!="gray")
					src<<output("<font color=[MessageColor]>You were hit for [Damage] vitality damage! (Protected from [100-(round(Damage/StartDamage)*10)]%)</font>","Attack")
	...
	if(WhereItHits=="Stamina")
		if(istype(src,/mob/IndigoRingBarrier))
			...
			src.Death(WhoDidIt)
			return
		if(istype(src,/mob/Bugs/Kekkai)||istype(src,/mob/Kibaku)||istype(src,/mob/Clone/WoodClone)||istype(src,/mob/Bugs/Swarm)||istype(src,/mob/Bugs/BugShield)||istype(src,/mob/Bugs/SpecialSwarm)||istype(src,/mob/Sand/SunaNoTate)||istype(src,/mob/Jutsu/Elemental/Shoton/CrystalMirror))
			src.health-=Damage
			src.Death(WhoDidIt)
			return
		if(src.GateIn=="Wound")
	...
	if(src.Buff<1000)
		if(prob(1))
			src.Buff+=pick(0.5,0.6,0.7,0.8,0.9,1)
	src.Death(WhoDidIt)
...
atom/movable/proc/DamageProc(Damage,WhereItHits,WhoDidIt,JutsuName,MessageColor="gray",JutsuType)
	...
	if(ismob(src))
		...
		if((src&&WhoDidIt)&&(WhoDidIt:Phasing||src:Phasing)&&(!src:knockedout))
			...
			return
		if((src&&WhoDidIt)&&(WhoDidIt:AFK||src:AFK)&&(!src:knockedout))
			src<<"[WhoDidIt] is AFK."
			WhoDidIt<<"They're AFK."
			return
