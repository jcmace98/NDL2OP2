...
mob/proc/ToggleSageAura()
	if(src.SageMode!="Rikudou")
		...
		src.AuraOn=0;src.overlays-='SageModeAura.dmi';src.overlays-='AshuraSageMode.dmi';src.overlays-='AshuraSageJacket.dmi'
		src.overlays-='SageModeAura.dmi';src.overlays-='New Icons(Con4)/Rikudou Truth Seeking Orbs.dmi';src.overlays-='RikudouAura.dmi';src.overlays-='PureYellowShoe.dmi'
		FTG_Mode=0
		src<<"You stop showcasing your Sage Chakra."
	else
		src.AuraOn=1
		src.overlays+='AshuraSageMode.dmi'
		src.overlays+='AshuraSageJacket.dmi'
		src.overlays+='SageModeAura.dmi'
	//	src.overlays+='RikudouAura.dmi'
		src.overlays+='PureYellowShoe.dmi'
...
mob/proc/RikudouSageMode(TimeLimit=15,Length=900)
	...
	while(SageModeTime&&src.SageMode!="")
		if(src.SageMode=="Rikudou"&&SageModeTime&&src.ChakraPool>0&&!src.knockedout&&!src.Dead)
			if(src.health<src.maxhealth)
				src.health+=65;src.chakra-=90
				src.SageModeTime-=1
			if(src.stamina<src.maxstamina)
				src.stamina+=(src.maxstamina/33);src.chakra-=90
				src.SageModeTime-=1
			if(src.chakra<1000)
				src.chakra+=200;src.ChakraPool-=100
			if(src.ChakraPool<2000)
				src.ChakraPool+=1000
		if(!UsedBijuu)
			sleep(7)
		sleep(4)
...
mob/proc/Kawa(Number)
	...
	if(Number>300)
		L.Controllable=1
	L.icon_state="running";L.Running=1;L.Age=0;L.maxstamina=1;L.Owner=src;L.name=src.name;L.icon=src.icon;L.dir=src.dir;L.overlays=src.overlays;L.loc=locate(src.x,src.y,src.z)
 /*walk(L,0)*/
	spawn(Number)
		if(L)
			L.Death(Number)
...
mob/Klog
	...
	Death(mob/M,TBD)
		spawn(1)
			...
			sleep(TBD/10)
			src.overlays=null;src.icon="NarutoStuff.dmi";src.icon_state="Log";var/obj/SmokeCloud/S = new /obj/SmokeCloud;S.loc=locate(src.x,src.y,src.z);src.Running=1
			sleep(15)
			del(src)
		..()
mob/Klog2
	...
	Death(mob/M)
		spawn(1)
			src.overlays=null;src.icon="NarutoStuff.dmi";src.icon_state="Exploding Log";var/obj/SmokeCloud/S = new /obj/SmokeCloud;S.loc=locate(src.x,src.y,src.z);src.Running=1
			sleep(5)
			...
			while(src)
				...
				if(src.icon_state=="fist")
					for(var/mob/M in src.loc)
						...
						if(M==O)
							return
						if(M.sphere|M.intsu|M.ingat|M.Kaiten||M.cwheel)
							del(src)
						if(M.FrozenBind=="")
...
mob/proc/SageMode(TimeLimit,Length,WhichAnimal)
	...
	while(A>1)
		src.chakra-=10
		if(src.icon_state!="rest")
			src<<"You stop concentrating!";A=0;src.Frozen=0;src.firing=0;src.meditating=0;return
			for(var/obj/SkillCards/SageModeApe/Z in src.LearnedJutsus)
				spawn() Z.DelayIt(150,usr)
			for(var/obj/SkillCards/SageModeToad/Z in src.LearnedJutsus)
				spawn() Z.DelayIt(150,usr)
			for(var/obj/SkillCards/SageModeLion/Z in src.LearnedJutsus)
				spawn() Z.DelayIt(150,usr)
			for(var/obj/SkillCards/SageModeSenju/Z in src.LearnedJutsus)
				spawn() Z.DelayIt(250,usr)
			for(var/obj/SkillCards/SageModeSlug/Z in src.LearnedJutsus)
				spawn() Z.DelayIt(150,usr)
			for(var/obj/SkillCards/SageModeDragon/Z in src.LearnedJutsus)
				spawn() Z.DelayIt(150,usr)
		if(src.icon_state=="rest")
			src.Frozen=1;src.firing=1;src.meditating=1
		A--
		sleep(11)
	src.icon_state=""
	src.Frozen=0
	...
	if(src.SageMode=="Senju")
		src.overlays+='Icons/New Base/SenjuSage.dmi'
	if(src.SageMode=="Toad")
		src.LearnedJutsus+= new/obj/SkillCards/SpirallingSpheres
		src.overlays+='Icons/New Base/ToadSageEyes.dmi'
	...
	if(src.SageMode=="Slug")
		src.chakra+=rand(4500,7500)
		if(src.chakra>src.Mchakra*1.5)
			src.chakra=src.Mchakra*1.5
		src.icon='Icons/New Base/SageSlug.dmi'
	if(src.SageMode=="Snake")
		src.icon='Icons/New Base/DragonSageMode.dmi'
		src.LearnedJutsus+= new/obj/SkillCards/BodyShedding
	...
	while(SageModeTime&&src.SageMode!="")
		if(src.SageMode=="Snake")
			...
			while(src.SageMode=="Snake"&&WearOffTime&&src.ChakraPool>0&&!src.knockedout&&!src.Dead)
				if(src.health<src.maxhealth/3)
					src.health+=33;src.ChakraPool-=65
					WearOffTime-=3
				if(src.stamina<src.maxstamina/3)
					src.stamina+=(src.maxstamina/45);src.ChakraPool-=65
					WearOffTime-=3
				if(src.chakra<1000)
					src.chakra+=200;src.ChakraPool-=100
				WearOffTime--
		...
		if(src.SageMode=="Monkey")
			...
			src.gen=src.Mgen*1.5
			src.tai=src.Mtai*2.1
		if(src.SageMode=="Senju")
			WearOffTime+=300
			src.nin=src.Mnin*2
			src.gen=src.Mgen*2
			src.tai=src.Mtai*2
			src.SturdyComposition=30
			while(src.SageMode=="Senju"&&WearOffTime&&src.ChakraPool>0&&!src.knockedout&&!src.Dead)
				if(src.health<src.health/2)
					src.health+=35;src.ChakraPool-=45
					WearOffTime-=2
				if(src.stamina<src.maxstamina/2)
					src.stamina+=(src.maxstamina/50);src.ChakraPool-=45
					WearOffTime-=2
				if(src.chakra<1000)
					src.chakra+=200;src.ChakraPool-=100
				WearOffTime--
		...
		if(src.SageMode=="Lion")
			...
			src.gen=src.Mgen*1.8
			src.tai=src.Mtai*2
			src.FireChakra=25;src.EarthChakra=25
		if(src.SageMode=="Slug")
			src.tai=src.Mtai*1.4
			src.nin=src.Mnin*1.6
	...
	for(var/obj/SkillCards/SpirallingSpheres/Z in src.LearnedJutsus)
		del(Z)
	for(var/obj/SkillCards/BodyShedding/Z in src.LearnedJutsus)
		del(Z)
	src.overlays-='Icons/New Base/SenjuSage.dmi'
	src.overlays-='Icons/New Base/SageMonkeyTail.dmi'
	src.overlays-='Icons/New Base/ToadSageEyes.dmi'
...
mob/proc/CursedSealActivation()
	if(src.CursedSealActivated)
		...
		src<<"You deactivate the Curse Mark!"
		src.icon=usr.Oicon
		src.nin=src.Mnin

		src.overlays-='Icons/Jutsus/Bijuu/UraseCursedSeal.dmi'
	...
	//		src.deathcount++
		...
		src<<"You activate the Cursed Mark!"
	//	if(src.EnduranceAddOn>0)
	//		src.EnduranceAddOn=0
		if(src.CursedSealPool<1000)
			src.deathcount++
		else
			src.deathcount+=0.5
		...
		if(src.CursedSealPool>=1000)
			if(!(locate(/obj/SkillCards/CursedSealActivationS) in src.LearnedJutsus))
				src<<"The seal begins to bind to your soul!"
				src.JutsuInLearning="CursedSealActivationS"

		var/icon/CS='Icons/Jutsus/Bijuu/CurseMarkStage1.dmi'
		var/icon/Urase='Icons/Jutsus/Bijuu/UraseCursedSeal.dmi'
		...
		if(src.CursedSealType=="Hell")
			src.gen*=1.4;src.nin*=1.4;src.tai*=1.4
	...
	//		usr.EnduranceAddOn+=350
		...
		if(src.CursedSealType=="Air")//Speed type CS
			src.RunningSpeed=8
			src.tai=src.Mtai*1.3;src.nin=src.Mnin*1.2;src.gen=src.Mgen*1.2
			src.stamina+=src.maxstamina*0.25
		var/obj/I=new()
		...
		while(src.CursedSealActivated)
			if(src.CursedSealType=="Air")
				src.runningspeed=15
			if(src.CursedSealType=="Thunder")
				src.Endurance=450
			src.health-=(max((rand(15,30)-src.CursedSealPool),2))
			if(prob(1.5)&&src.CursedSealPool<1000)
				src.deathcount++
				src.CursedSealPool++
			else if(prob(0.5)&&src.CursedSealPool>1000)
				src.deathcount+=0.5
				src.CursedSealPool++
...
obj/CurseMarkWings
	icon='Icons/Jutsus/Bijuu/CurseSeal2Left.dmi'
	layer=MOB_LAYER
	A
		icon='Icons/Jutsus/Bijuu/CurseSeal2Left.dmi'
		pixel_x = -16
	B
		icon='Icons/Jutsus/Bijuu/CurseSeal2Right.dmi'
		pixel_x = 16
mob/proc/CursedSealActivationS()
	if(src.CursedSealActivated)
		src.CursedSealActivated=0
	if(src.CursedSealActivatedS)
		...
		if(src.StruggleAgainstDeath>0&&src.knockedout)
			src.Struggle=src.StruggleAgainstDeath+1
		if(src.CursedSealType=="Hell")
			src.icon='Icons/Jutsus/Bijuu/Heaven-EarthCS2(Skin).dmi'
		if(src.CursedSealType=="Heaven")
			src.icon='Icons/Jutsus/Bijuu/Heaven-EarthCS2(Skin).dmi'
			src.overlays+=/obj/CurseMarkWings/A;src.overlays+=/obj/CurseMarkWings/B
		if(src.CursedSealType=="Earth")
			src.icon='Icons/Jutsus/Bijuu/Heaven-EarthCS2(Skin).dmi'
		if(src.CursedSealType=="Fire")
			src.icon='Icons/Jutsus/Bijuu/FireCS2.dmi'
		if(src.CursedSealType=="Water")
			src.icon='Icons/Jutsus/Bijuu/WaterCS2.dmi'
		if(src.CursedSealType=="Thunder")
			src.icon='Icons/Jutsus/Bijuu/Thunder CS2.dmi'
		if(src.CursedSealType=="Air")
			src.icon='Icons/Jutsus/Bijuu/AirCS2.dmi'
/*		var/icon/CS='Icons/Jutsus/Bijuu/CurseMarkStage1.dmi'
		var/icon/Urase='Icons/Jutsus/Bijuu/UraseCursedSeal.dmi'
		src.CSLays=src.overlays.Copy()
		src.overlays=src.overlays.Remove(src.overlays)
		...
		if(src.CursedSealType=="Heaven")//Ninjutsu type CS
			src.gen=src.Mgen*2;src.nin=src.Mnin*2.1
			src.chakra+=(src.Mchakra*1.25)

		if(src.CursedSealType=="Earth")//Taijutsu type CS
			src.gen=src.Mgen*2;src.tai=src.Mtai*2.5;src.nin=src.Mnin*1.8
			src.stamina+=(src.maxstamina*0.5)

		if(src.CursedSealType=="Fire")//Accuracy type CS
			src.OldFocus=src.Focus;src.Focus*=3.2
			src.gen=src.Mgen*1.9;src.tai=src.Mtai*1.7;src.nin=src.Mnin*2.1
			src.chakra+=src.Mchakra*0.5

		if(src.CursedSealType=="Water")//Genjutsu type CS
			src.gen=Mgen*2.5;src.nin=src.Mnin*1.85
			src.chakra+=src.Mchakra*0.5

		if(src.CursedSealType=="Thunder")//Endurance type CS
			src.tai=Mtai*2.2;src.nin=src.Mnin*1.8;src.gen=src.Mgen*1.6
//			usr.EnduranceAddOn+=800
			src.stamina+=src.maxstamina*0.5
...
/*		if(src.CursedSealType=="Urase")//urase type cs
		...
		while(src.CursedSealActivatedS)
			...
			if(src.CursedSealType=="Thunder")
				src.Endurance=900
			src.health-=(max((rand(10,20)-src.CursedSealPool),2))
			if(prob(1.5)&&src.CursedSealPool<2000)
				src.deathcount++
				src.CursedSealPool++
			else if(prob(0.5)&&src.CursedSealPool>2000)
				src.CursedSealPool++
...
mob/proc
	Flying_Thunder_God_Mode()
		if(src.FTG_Mode)
			...
			src<<"Your muscles begin to tense..."
			sleep(25)
			src<<"Your reflexes are heightend."
			sleep(10)
