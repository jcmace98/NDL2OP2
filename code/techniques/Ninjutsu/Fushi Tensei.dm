				...
				spawn(300)
					...
					if(src.Struggle>M.Struggle)
						...
						for(var/obj/SkillCards/CursedSealActivation/A in src.LearnedJutsus)
							del(A)
						for(var/obj/SkillCards/CursedSealActivationS/A in src.LearnedJutsus)
							del(A)
						...
						src.ShotonKnowledge=M.ShotonKnowledge;src.ShotonMastery=M.ShotonMastery;src.CrystalChakra=M.CrystalChakra
...
//	var/Skillpoints=0
		...
		if(src.remaking)
			...
			if(B==3)
				...
				src.GenSkill+=15
			if(B==4)
				src.Trait3="Super Human Strength"
				src.TaiSkill+=5
			if(B==5)
			...
			if(src.Clan=="Uchiha")
				if(prob(50))
					...
					var/ASA=list("obito","itachi","madara","sasuke","star","6 point","gridlock","shuriken","shin")
