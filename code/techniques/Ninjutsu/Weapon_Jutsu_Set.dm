...
//	var/Mastery=0
	...
	T=0
	if(Check1==1)
		src<<"You've summoned some Kunais!"
	...
	//	var/obj/WEAPONS/Kunai/BBB=new();BBB.icon_state="Throwz";BBB.loc=locate(src.x-1,src.y-1,src.z)
		return
	if(Check1==4)
		src<<"You've summoned a hundred kunais!"
		for(var/obj/WEAPONS/Kunai/O in usr.contents)
			counter+=100
		if(counter<=0)
	...
	while(rounds>0&&src.icon_state=="handseal") //handseal icon state automatically freezes them because its coded that way
		...
		for(var/i=0, i<Size*2, i++)
			spawn()
				...
				K.InitialDisplacement=K.SetInitialDisplacement(-6,6)
				walk(K,src.dir)
		rounds--;
		sleep(4);//delay btw rounds
	src.icon_state=""
obj/KKunai
	icon='Kunai.dmi'
	icon_state = "Throwz"
	...
	Bump(A)
		if(ismob(A))
			var/mob/M = A
			var/mob/O=src.Owner
			var/damage=(O.KunaiMastery+(rand(10,30)+(src.nin*0.65))) //Was var/damage=(src.nin*0.4)+48
									   //The +48 insures the move does min of 28 damage.
			if(damage>100)//Caps the damage in case they use a nin buff transformation so move doesn't become oped.
				damage=100;
			var/random=(rand(6,8)/8)//Number changed if want to buff or nerf the damage.
