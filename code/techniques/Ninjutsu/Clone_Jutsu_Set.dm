	...
	Clone
		New()
			...
			spawn(900)
				del(src)
		Click()
			if(CloneType=="KBunshin"||CloneType=="KKBunshin"||CloneType=="TKageBunshin"||CloneType=="ShotonBunshin")
		...
		Bump(atom/M)
			if(istype(M,/mob/)) // If they run into the player
				if(M == src.Owner||M.name==src.name)
					...
					if(CloneType=="KBunshin"||CloneType=="TKageBunshin"||CloneType=="KKBunshin"||CloneType=="ShotonBunshin")
						var/mob/O=src.Owner
						var/mob/MX=M
						if(src.Tackling==1)
	...
	if(usr.Clan=="Inuzuka"||usr.Clan2=="Inuzuka")
		for(var/mob/pet/P in world)
			if(P.Owner == usr&&P.Frozen == 0&&P.FrozenBind=="")
				if(prob(5))
					usr.Training+=0.5;if(usr.Training>50) usr.Training=50
				if(prob(2))
					usr.SpeedTraining+=0.2;if(usr.SpeedTraining>5) usr.SpeedTraining=5
	...
	for(var/mob/Puppet/Hawk/K in world)
		if(K.Owner==usr&K.Stun<=0&&K.ControlType=="Mouse")//just added the K.stun part ;o
			if(K.Frozen)
				...
				walk_towards(K,src)
	for(var/mob/Clone/WoodClone/K in world)
		if(K.Owner==usr)
			if(K.Frozen)
				K.Frozen = 0
				walk_towards(K,src)
		...
		for(var/mob/pet/P in world)
			if(P.Owner == usr&&P.Frozen == 0&&P.FrozenBind=="")
				if(prob(2))
					usr.Training+=0.5;if(usr.Training>50) usr.Training=50
				if(prob(2))
					usr.SpeedTraining+=0.5;if(usr.SpeedTraining>5) usr.SpeedTraining=5
				walk_to(P,src,1,1)
			else
				..()
	if(usr.Clan=="Kumojin"||usr.Clan2=="Kumojin")
		for(var/mob/NPC/KumojinSpider/AK in world)
			if(AK.Owner==usr)
				if(AK.Frozen||AK.knockedout)
					...
					walk_to(AK,src)
	for(var/mob/Clone/WoodClone/B in world)
		if(B.Owner==usr)
			walk_to(B,src,,1)
	..()
	...
	if(usr&&usr.ShurikenMode)
		for(var/obj/WEAPONS/Shuriken/A in view(10,usr))
			if(A.icon_state=="throw")
				walk_to(A,src)
				usr.chakra-=8
	if(usr&&usr.Clan=="Aburame"||usr&&usr.Clan2=="Aburame")
		for(var/mob/Bugs/Kekkai/K in usr.BugsInWorld)
			if(K.Frozen)
				K.Frozen = 0
				walk_to(K,src)
		...
		for(var/mob/Bugs/SpecialSwarm/S in usr.BugsInWorld)
			if(S.Controllable)
				walk_to(S,src)
	if(usr&&usr.Clan=="Inuzuka"||usr&&usr.Clan2=="Inuzuka")
		for(var/mob/pet/P in world)
			if(P.Owner == usr&&P.Frozen == 0&&P.FrozenBind=="")
				if(prob(2))
					usr.Training+=0.5;if(usr.Training>50) usr.Training=50
				...
				..()
	if(usr&&usr.Clan=="Kumojin"||usr&&usr.Clan2=="Kumojin")
		for(var/mob/NPC/KumojinSpider/AK in world)
			if(AK.Owner==usr)
				if(AK.Frozen||AK.knockedout)
					return
