...
*/
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(O.Trait=="Old Powerful"||O.SecondName=="Bayamo") damage+=(O.NinSkill*2)
			...
			//				src.SoundEngine('SFX/WindSFX.wav',6)
					...
					for(var/mob/M in src.loc)
						if(M.Kaiten||M.sphere||M.Mogu)
							return
						var/damage=round(rand(src.nin*20,src.nin*35))//22 and 35 if I decide to change it back
						damage=round(((damage*1.15)*(WindChakra*0.02+0.8)));
						damage=damage/(M.Endurance/2.5)//Was /2
						if(O.Trait=="Old Powerful"||O.SecondName=="Bayamo")
							damage+=(O.NinSkill)
						if(damage>150)
							damage=150
	...
	Kakeami
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(O.Trait=="Old Powerful"||O.SecondName=="Bayamo") damage+=(O.NinSkill*0.5)
	...
	VacuumSerialWaves
		...
		//	..()
			if(ismob(A))
				...
				if(O&&O.Trait=="Old Powerful"||O.SecondName=="Bayamo")
					damage+=(O.NinSkill)
	...
	FuutonRenkuudan
		...
		Bump(A)
			...
			if(ismob(A))
				...
				var/mob/O=src.Owner
				var/damage=round(rand(src.nin*300,src.nin*400))
				var/WindChakra=WindDamage(O);damage=round(damage*(WindChakra*0.02+1.3))
				damage=damage/(M.Endurance/7.5)
				if(O.Trait=="Old Powerful"||O.SecondName=="Bayamo") damage+=(O.NinSkill*3)
					...
					spawn(1) for(var/mob/M in view(Range,src))
						spawn()
							if(M.Mogu||M.sphere||M.Kaiten)
								...
								var/damage=round((src.nin*rand(20,30))*((WindChakra*0.02)+1.1));//was +1.3; Raised the Usage of Nin for the damage, making it much more cap based.
								var/Chakraz=M.ChakraArmor*0.01;
								damage=round(damage)*(O.WindChakra*0.055);//*0.1
	...
	GreatSphere
		...
		Bump(A)
			...
			if(ismob(A))
				...
				damage+=src.nin
				damage=round(damage*(WindChakra*0.02+1.1))
				if(O.Trait=="Old Powerful"||O.SecondName=="Bayamo") damage+=(O.NinSkill)
	...
	AirBall
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(O.Trait=="Old Powerful"||O.SecondName=="Bayamo") damage+=(O.NinSkill*5)
...
mob/proc
	AirDash()
		if(src.Frozen||src.FrozenBind!=""||src.Shibari)
			return
	...
	Gust()
		...
		if(src.HandsSlipped||src.Shibari) return
		...
		spawn(15)
			...
			for(var/obj/Jutsu/Elemental/Fuuton/Gust/C in world)
				if(C.Owner==src)
					del(C)
	Whirlwind()
		if(src.InGarou||src.FrozenBind||src.Shibari)
			return
		else
	...
	KazeNoYaibaJutsu()
		if(usr.firing||usr.Frozen||usr.FrozenBind!=""||src.Shibari)
			return
		else
			if(src.target)
	...
	//	if(src.Bijuu==""&&src.Bijuu!="Shukaku")
		...
		//	if(src.HandsSlipped) return
		if(src.Bijuu=="Ichibi"&&src.TailState>=0.5)
			var/obj/Jutsu/Elemental/Fuuton/FuutonRenkuudan/A=new();A.loc=get_step(src,turn(src.dir,45));A.name="[usr]";A.Owner=usr;walk(A,usr.dir);A.nin=src.nin*4;A.Move_Delay=(0)//sleep(10)
			var/obj/Jutsu/Elemental/Fuuton/FuutonRenkuudan/C=new();C.loc=get_step(src,turn(src.dir,-45));C.name="[usr]";C.Owner=usr;walk(C,usr.dir);C.nin=src.nin*4;C.Move_Delay=(0)
		...
		if(usr.Bijuu=="Ichibi"&&src.TailState>=0.5)
			K.nin*=1.5
		K.Move_Delay=(0);//if(src.WindVelocity>=3) K.Move_Delay=0
	FuutonVacuumCannon()
