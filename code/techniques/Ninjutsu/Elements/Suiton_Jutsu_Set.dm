...
obj/Jutsu/Elemental/Suiton
	...
	MizuTeppo
		...
		Bump(A)
			...
			if(ismob(A))
				...
				damage+=AddOn
				if(O.Trait=="Old Powerful"||O.SecondName=="Hozuki") damage+=(O.NinSkill*2)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
	...
	//			var/mob/O = src.Owner
				...
				if(O.Trait=="Old Powerful"||O.SecondName=="Hozuki") damage+=(O.GenSkill*8)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
				if(damage>1500)
					damage=1500
	...
	Mizurappa
		...
		Bump(A)
			...
			if(ismob(A))
				...
				var/AddOn=(O.WaterPressure*0.150)*damage
				damage+=AddOn
				if(O.Trait=="Old Powerful"||O.SecondName=="Hozuki") damage+=(O.NinSkill*8)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
				if(M.PaperStyleDance)
					M.PaperStyleDance=0
	...
	Suiryedan
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(damage>2000)
					damage=2000
				if(O.Trait=="Old Powerful"||O.SecondName=="Hozuki") damage+=(O.NinSkill*7)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
				if(M.PaperStyleDance)
					M.PaperStyleDance=0
		...
		//	var/mob/O=src.Owner
			spawn()//Added Recently
				while(src)
					sleep(1)
					var/mob/O=src.Owner
					if(src.WaterCheck1()&&O.Clan=="Shiroi"||O.Clan2=="Shiroi")
	...
	/*	Move()
		...
		Bump(A)
			...
			if(ismob(A))
				...
				//damage+=AddOn
				if(O.Trait=="Old Powerful"||O.SecondName=="Hozuki") damage+=(O.NinSkill*5)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
				if(damage>1000)
					damage=1000
	...
	Teppoudama
		...
		Bump(A)
			...
			if(ismob(A))
				...
				var/AddOn=(O.WaterPressure*0.150)*damage
				damage+=AddOn
				if(O.SecondName=="Hozuki") damage+=(O.NinSkill*5)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
				if(M.PaperStyleDance)
					M.PaperStyleDance=0
			...
			New()
				...
				//				del(P)
						...
						for(var/mob/M in src.loc)
							...
							if(M.Clan=="Hoshigaki")
								damage*=0.7
							if(O.Trait=="Old Powerful"||O.SecondName=="Hozuki")
								damage=damage*1.5
							M.DamageProc(damage,"Stamina",O,"Daibakufu","#8088FF")
							if(M.RaiArmor)
								M.StunAdd(10);M.DamageProc(500,"Health",O)
			...
			New()
				...
				//				del(P)
						...
						for(var/mob/M in src.loc)
							...
							var/AddOn=(O.WaterPressure*0.150)*damage
							damage+=AddOn
							if(O.SecondName=="Hozuki") damage+=(O.NinSkill*6)
							if(M.Clan=="Hoshigaki")
								damage*=0.7
							M.DamageProc(damage,"Stamina",O,"Daibakufu","#8088FF")
							if(M.RaiArmor)
	...
	WaterWave
		...
		Bump(A)
			...
			//	view(M)<<output("<font color=#8088FF>[M] has been hit by the Baku Suishou for [damage] damage!</font>","Attack")
				if(M.PaperStyleDance)
					M.PaperStyleDance=0
				if(O.SecondName=="Hozuki") damage+=(O.NinSkill*4)
				if(M.Clan=="Hoshigaki")
					damage*=0.7
			//	M.DamageProc(damage,"Stamina",O)
...
mob/proc
	CreateWater()
		...
		if(src.SuitonKnowledge>1500)
			BonusWater=3
		if(src.SuitonKnowledge>3000)
			BonusWater=10
	//	if(src.SuitonKnowledge>10000)
	//		BonusWater=25
