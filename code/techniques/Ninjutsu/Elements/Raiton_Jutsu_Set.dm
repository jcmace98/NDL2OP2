	...
	Raikyuu
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(O.Trait=="Old Powerful"||O.SecondName=="Ryouken") damage+=(O.NinSkill*3)
				if(M.RaiArmor)
			...
			if(istype(A,/obj/))
				...
				if(istype(ZZ,/obj/Jutsu/Elemental/Doton/))
					...
					if((ZAC.JutsuLevel/2)>=src.JutsuLevel)
						...
						del(ZAC)
				if(istype(ZZ,/obj/Jutsu/Elemental/Shoton/))
					var/obj/Jutsu/Elemental/Shoton/ZAC=ZZ
					if((ZAC.JutsuLevel/3)>=src.JutsuLevel)
						del(src)
					else
						del(ZAC)
			...
			spawn()
				while(src)
					...
					for(var/obj/Jutsu/Elemental/P in src.loc)
						...
						if(istype(P,/obj/Jutsu/Elemental/Shoton))
							if((P.JutsuLevel/3)>src.JutsuLevel) del(src)
							else del(P)
			...
			if(ismob(A))
				...
				if(M.RaiArmor)
					if((M.RaitonKnowledge-O.RaitonKnowledge)>damage)
						...
						if((damage-resistance)<0) resistance=(damage-1)
						damage-=resistance
				if(O.Trait=="Old Powerful"||O.SecondName=="Ryouken") damage+=(O.NinSkill/2)
				if(M.Clan=="Hoshigaki")
					damage*=1.2
				if(O.Electromagnetivity>0&&prob(10*O.Electromagnetivity*(M.InHydro?3:1)))
					if(!M.ImmuneToDeath)
...
*/
		...
		//		view(M)<<output("<font color=yellow size=2>[M] was hit by Ikazuchi No Kiba!([(damage)])</font>","Attack")
			...
			if(istype(A,/obj/))
				...
				if(istype(ZZ,/obj/Jutsu/Elemental/Shoton/))
					var/obj/Jutsu/Elemental/Shoton/ZAC=ZZ
					if((ZAC.JutsuLevel)>=src.JutsuLevel)
						del(src)
					else
						del(ZAC)
				if(istype(ZZ,/obj/Jutsu/Elemental/Suiton/))
				...
				while(src)
					...
					spawn(1)
						...
						for(var/obj/Jutsu/Elemental/P in oview(1,src))
							...
							if(istype(P,/obj/Jutsu/Elemental/Shoton))
								if((P.JutsuLevel)>src.JutsuLevel) del(src)
								else del(P)
							if(istype(P,/obj/Jutsu/Elemental/Suiton))
								var/obj/Jutsu/Elemental/Suiton/A=P
								if(!A.Electrocuted)
		...
		New()
			...
			spawn(1)
				...
				while(src)
					...
					for(var/obj/Jutsu/Elemental/Doton/Z in src.loc)
						if((Z.JutsuLevel)>src.JutsuLevel) del(src)
						else del(Z)
					for(var/obj/Jutsu/Elemental/Shoton/Z in src.loc)
						if((Z.JutsuLevel)>src.JutsuLevel) del(src)
						else del(Z)
					for(var/obj/Jutsu/Elemental/Suiton/Z in src.loc)
						if(!Z.Electrocuted)
		...
		while(src.RaiArmor&&!src.sphere)
			...
			if(src.FrozenBind=="Sand")
				...
				for(var/obj/Jutsu/Elemental/Doton/DorouDomu/A in src.loc)
					del(A)
				for(var/obj/Jutsu/Elemental/Shoton/CrsytalCrimsonFruit/A in src.loc)
					del(A)
			sleep(11)
	Raiton_Armor(Uses)
		if(src.Shibari)
...
obj/Jutsu/Elemental/Raiton/Prisonn
	icon='Icons/Jutsus/RaitonTechniques.dmi'
	icon_state="shield"
	layer=5
	var/mob/Binding
...
mob/proc
	RaitonPrison()
		...
		if(src.firing)
			return
		if(!M)
			for(var/obj/SkillCards/RaitonPrison/A in usr.LearnedJutsus)
				A.DelayIt(25,usr,"Element")
			src << "You need a target";goto CoolDown
		for(var/obj/Jutsu/Elemental/Raiton/Prisonn/P in world)
			if(P.Owner==src)
				del(P)
		...
		var/obj/Jutsu/Elemental/Raiton/Prisonn/P = new();
		P.Owner=src;
		P.loc=M.loc;
		P.Binding=M
		M.FrozenBind="Prisonn"
		src.FrozenBind="Prisonn"
		src.icon_state="handseal"
		M.Frozen=1
		src.Frozen=1
		M << output("<b><font color=yellow>You've been caught in [usr]'s Raiton Prison Jutsu!</font></b>","Attack")
		M << "Quick, tap space to Struggle!"
		var/Damage=src.nin*1.5
		M.Struggle=0
		while(src.icon_state=="handseal")
			if(M.InHydro)
				M<<"You absorb the lightning which [src] forms around you!"
				usr<<"!?The water prison is absorbed into [M]!?"
				if(M.health<M.maxhealth)
					M.Stun += 50
					M.DamageProc(50,"Stamina",src)
		...
		M.Frozen=0
		M.firing=0
		for(var/obj/Jutsu/Elemental/Raiton/Prisonn/Objects in world)
			if(Objects.Owner==src)
				del(Objects)
				del(P)
		CoolDown
		for(var/obj/SkillCards/RaitonPrison/S in usr.LearnedJutsus)
