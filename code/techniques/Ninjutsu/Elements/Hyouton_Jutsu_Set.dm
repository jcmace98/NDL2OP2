					...
					for(var/mob/M in src.loc)
						if(M in OnlinePlayers)
							...
							if(M.FrozenBind=="")
								...
								M.FrozenBind="DoBind"
							if(M==O||M.Mogu||M.sphere||M.intsu||M.cwheel||M.ingat||M.Kaiten||M.RaiArmor||M.intank)
								return
							if(M.SusanooIn)
					...
					if(!(ismob(T)))
						if(istype(T,/turf/water)||istype(T,/turf/water2)||istype(T,/obj/Jutsu/Elemental/Suiton/Owater))
							if(prob(40))
								...
								spawn(15)
									var/obj/Jutsu/Elemental/Hyouton/ISawarabi/A=new/obj/Jutsu/Elemental/Hyouton/ISawarabi(locate(AB.x,AB.y,AB.z))
									var/obj/Jutsu/Elemental/Hyouton/ISawarabi/C=new/obj/Jutsu/Elemental/Hyouton/ISawarabi(locate(AB.x,AB.y,AB.z))
									var/Z=rand(1,7)
									A.Owner=src;flick("rise[Z]",A);C.Owner=src;flick("rise[Z]",C)
									if(Z==1||Z==2||Z==3)
										A.icon_state="spike[Z]";C.icon_state="spike[Z]"
									else
										A.icon_state="Spike[Z]";C.icon_state="Spike[Z]"
									A.density=rand(0,1);C.density=rand(0,1)
				num++
				if(num<7)
					goto StartSpikes
			else
