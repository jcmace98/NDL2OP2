			...
			//				if(O.RaitonKnowledge>E.JutsuLevel)
				...
				//		M.Defused=1
					...
					//	spawn()
						...
						//		step_away(M,O)
						//		step_away(M,O)
								var/damage=45//Might change the damage
							...
							//	M.DamageProc(damage,"Health",O)
								...
								if(prob(45))//was originally 30
									M.StunAdd(30)
							else
								M.StunAdd(15)
								var/Damage=src.nin/1.6
								if(Damage>225)
									Damage=225
	...
	ChidoriSenbon
		...
		//				del(src)
					for(var/obj/Jutsu/Elemental/Doton/Z in oview(1,src))
						if(prob(20))
							del(Z)
					for(var/obj/Jutsu/Elemental/Shoton/Z in oview(1,src))
						if(prob(15))
							del(Z)
					for(var/obj/Doors/D in oview(1,src))
						D.DamageProc(src.nin/10,"Health")
...
mob/proc
	...
	//	var/Check=0
		...
		spawn(30)
			src.icon_state="";src.Stun=1;src.firing=0
		sleep(5)
		if(src.Guarding&&src.dir!=NORTHEAST&&src.dir!=SOUTHEAST&&src.dir!=SOUTHWEST&&src.dir!=NORTHWEST)
			src.icon_state="beastman";var/Number=round(src.NagashiExpert*10)
	...
	//	var/Check=0
		...
		if(src.Guarding&&src.dir!=NORTHEAST&&src.dir!=SOUTHEAST&&src.dir!=SOUTHWEST&&src.dir!=NORTHWEST)
			...
			if(Number>3) Number=3;Number2+=(src.NagashiExpert-Number)
			src.ChakraDrain(5550*Number)
			var/obj/Jutsu/Elemental/Raiton/ChidoriNagashi/A=new();A.loc=src.loc;A.Owner=src;A.JutsuLevel=src.RaitonKnowledge;A.nin*=Number2
			A.Owner=src;A.loc=src.loc;A.layer = MOB_LAYER-1;
			src.F.Add(A);
			while(N<4&&src.icon_state=="Power")
				for(var/turf/T in oview(N,src))
					if(!(T in oview(N-1)))
						var/obj/Jutsu/Elemental/Raiton/ChidoriNagashi/B=new();B.loc=T;B.Owner=src;B.JutsuLevel=src.RaitonKnowledge;B.nin*=Number2
						src.F.Add(B)
				N++
		//		sleep()
		...
		if(src.firing)
			...
			if(I>6)
				...
				if(src.ChidoriSenbonExpert>4) // One Time Fix for Overcapping
					src.ElementalPoints+=(src.ChidoriSenbonExpert-4);src.ChidoriSenbonExpert=4
			while(I>0)
				var/obj/Jutsu/Elemental/Raiton/ChidoriSenbon/A=new();A.Owner=src;A.JutsuLevel=src.RaitonKnowledge;A.nin=src.nin
				A.loc=src.loc
				if(prob(45))
					if(prob(50))
				...
				A.pixel_x=rand(-2,16);A.pixel_y=rand(0,0)
				spawn(60)
					del(A)
				I--
