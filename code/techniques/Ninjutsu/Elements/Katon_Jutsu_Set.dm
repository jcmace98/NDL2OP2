...
obj/Jutsu/Elemental/Katon/F
	icon='Pyro.dmi'
	icon_state="Pyro";
obj/Jutsu/Elemental/Katon/Drill
	icon='KatonDrill.dmi'
	icon_state="";
...
obj/Jutsu/Elemental/Katon
	...
	KatonDrill//Needs to delete itself after a time.. idk how to do that.
		icon='Icons/Jutsus/KatonDrill.dmi'
		layer = MOB_LAYER+4
		density=1
		dir=SOUTH
		var/nin=10
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(M.Kaiten) return
				var/mob/O=src.Owner
				var/damage=round((O.nin*0.3)+20)
				M.DamageProc((damage*(5/10)),"Stamina",O,"drill","red")
			if(istype(A,/obj/))
				if(istype(A,/obj/Jutsu/Elemental/Katon/KatonDrill))
					...
					del(src)
		Del()
			var/obj/Jutsu/Elemental/Katon/KatonDrill/A=new();A.dir=src.dir;A.nin=src.nin/2;A.Owner=src.Owner;..()
...
/*
	...
	//						W.BurnWire()
					...
					for(var/mob/M in oview(DamageRangingField,src))
						spawn()
							if(!M)
								continue
							if(M.Kaiten||M.sphere||M.UsingDomu||M.UsingCDome||M.Mogu||M.InvulnerableToFire)
								return
							if(M.DotonArmor)
							...
							if(M.Ghit<=0)
								...
								if(M.Clan=="Ketsueki"||M.Clan=="Hoshigaki"||M.Clan2=="Ketsueki"||M.Clan2=="Hoshigaki")
									if(!M.InHydro)
										if(M.Clan=="Ketsueki"||M.Clan2=="Ketsueki")
											damage=damage*1.25
											ChanceOfBurn*=1.5
										else if(M.Clan=="Hoshigaki"||M.Clan2=="Hoshigaki")
						...
						//		if((M.Clan=="Ketsueki" && !M.InHydro)||(M.Clan=="Hoshigaki"&& !M.InHydro))
							...
							//			return
									...
									if(FireImmune>5)
										FireImmune=5
									damage-=(damage*(FireImmune*0.1))//(FireImmune*0.08)
									if(damage<0)
										damage=5
						//		if(M.Bijuu=="Nibi")
				...
				//						return
								...
								spawn()
									if(O.SecondName=="Crimson")
										M.DamageProc((damage*(3.5/5)),"Health",O,"flames","red","Heat")
								if(M)
									if(O.Magma)
										M.Ghit=4//was 3
									else
		...
		New()
			...
			//			src.SoundEngine('FireSFX.wav',5,40)
					...
					for(var/mob/M in oview(DamageRangingField,src))
						spawn()
							...
							if(M.Ghit<=0)
								...
								if(M.NibiResistance==3)
									damage /=10
								if(O.Trait=="Old Powerful"||O.SecondName=="Crimson") damage+=(O.NinSkill/3)
								if(M.Clan=="Ketsueki" && !M.InHydro)
									damage=damage*1.25
									ChanceOfBurn*=1.5
								damage-=(Chakraz*damage)
						...
						//			waterDeduction+=3
							...
							//	view(M)<<output("<font color=red>[M] has been hit by the flames for [damage] damage!</font>","Attack")
								spawn()
									M.DamageProc((damage*(1/3)),"Stamina",O,"flames","red")
								spawn()
									if(M&&!M.BoneMembrane)
										M.DamageProc((damage*(2/4)),"Health",O,"flames","red")
								if(M)
									if(O.Magma)
										M.Ghit=3
									else
					...
					for(var/obj/Doors/D in oview(1,src))
						...
						sleep(1)
					for(var/obj/Jutsu/Elemental/E in oview(DamageRangingField,src))
						var/damage=round(src.nin*pick(0.6,1.5));damage=round(damage*(O.FireChakra*0.02+0.8))
						if(O.Trait=="Old Powerful"||O.SecondName=="Crimson") damage+=(O.NinSkill/3)
						if(E.Element=="Mokuton"||E.Element=="Hyouton")
							damage*=2
						if(E.Element=="Doton")
			...
			if(ismob(A))
				...
				if(M.InHydro)
					...
					if(prob(40))
						src.loc=null
						return
				M.DamageProc((damage*(3/6.5)),"Stamina",O,"fireball","red")
				if(M&&!M.BoneMembrane)
					M.DamageProc((damage*(2/6.5)),"Health",O,"fireball","red")
				else if(M)
					M.DamageProc((damage*(1/6.5)),"Health",O,"fireball","red")
	...
	SmallFireExplosion
		...
		New()
			...
			//		src.CreateSmoke("Light")
				...
				//		waterDeduction+=3
					for(var/mob/M in oview(1,src))
						if(!M.UsingDomu&&!M.Kaiten&&!M.UsingCDome)
							var/mob/O=src.Owner
							var/damage=O.nin*rand(1.5,1.8)
							if(M.InHydro)
								damage /= 15
							if(M.NibiResistance==1)
								damage /=5
	...
	KRyuuka
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(M.NibiResistance==3)
					damage /=10
				M.DamageProc((damage*(1.5/6.5)),"Stamina",O,"Ryuuka","red")
				if(M&&!M.BoneMembrane)
					M.DamageProc((damage*(3.5/6.5)),"Health",O,"Ryuuka","red")
				...
				//		waterDeduction+=3
					...
					for(var/mob/M in oview(DamageRangingField,src))
						...
						if(M.Ghit<=0&&M!=O)
							...
							if(M.Clan=="Ketsueki" && !M.InHydro)
								damage=damage*2.5
							if(O.Trait=="Old Powerful"||O.SecondName=="Crimson") damage+=(O.NinSkill/2)
							if(M.InHydro)
								damage /= 15
							if(M.NibiResistance==1)
								damage /=5
			...
			if(ismob(A))
				...
				if(M.InHydro)
					...
					if(prob(40))
						src.loc = null
						return
				M.DamageProc((damage*(2/6.5)),"Stamina",O,"Houka","red")
				if(M&&!M.BoneMembrane)
					M.DamageProc((damage*(3/6.5)),"Health",O,"Houka","red")
			...
			//		waterDeduction+=3
				for(var/mob/M in oview(DamageRangingField,src))
					...
					if(M.PaperStyleDance && !M.InHydro)
						M.PaperStyleDance=0
					if(M&&O&&M.Ghit<=0&&M!=O)
						var/damage=round(src.nin*2);damage=round(damage*(O.FireChakra*0.02+0.6));var/Chakraz=M.ChakraArmor*0.01
						if(M.Clan=="Ketsueki" && !M.InHydro)
							damage=damage*2
						if(O.Trait=="Old Powerful"||O.SecondName=="Crimson") damage+=(O.NinSkill/2)
						if(M.NibiResistance==1)
	...
	Gouenkyuu
		...
		var/nin=10
		var/Pushes=0
		var/PushCap=11//This is how many times it pushes the person backfore exploding
		New()
			..()
			var/mob/O=src.Owner
			if(O&&O.BlueFire)
				...
				//					M.Frozen=1
						...
						for(var/mob/M in oview(DamageRangingField,src))
							...
							if(M.PaperStyleDance && !M.InHydro)
								M.PaperStyleDance=0
							if(M&&O&&M.Ghit<=0&&M!=O)
								var/damage=round(src.nin*2.1);damage=round(damage*(O.FireChakra*0.22+0.65));var/Chakraz=M.ChakraArmor*0.01
								if(M.Clan=="Ketsueki" && !M.InHydro)
									damage=damage*2
								if(O.Trait=="Old Powerful"||O.SecondName=="Crimson") damage+=(O.NinSkill/2)
								if(M.NibiResistance==1)
			...
			//				var/obj/Jutsu/Elemental/Katon/Goukakyuu/AA=new();AA.loc=locate(A.x,A.y,A.z);AA.nin=src.nin;AA.Owner=src.Owner;del(A)
						...
						for(var/obj/Jutsu/Elemental/Fuuton/Z in oview(DamageRangingField,src))
							if(Z.Ignitable==1)
								...
								var/obj/Jutsu/Elemental/Katon/Goukakyuu/AA=new();var/obj/Jutsu/Elemental/Katon/Goukakyuu/K=new();var/obj/Jutsu/Elemental/Katon/Goukakyuu/B=new();var/obj/Jutsu/Elemental/Katon/Goukakyuu/C=new();var/obj/Jutsu/Elemental/Katon/Goukakyuu/D=new()
								AA.loc=locate(src.x+1,src.y,src.z);K.loc=src.loc;B.loc=locate(src.x-1,src.y,src.z);C.loc=locate(src.x,src.y+1,src.z);D.loc=locate(src.x,src.y-1,src.z)
								AA.nin=src.nin*1.9;K.nin=src.nin/2;B.nin=src.nin/2;C.nin=src.nin/2;D.nin=src.nin/2
								AA.Owner=src.Owner;K.Owner=src.Owner;B.Owner=src.Owner;C.Owner=src.Owner;D.Owner=src.Owner
								del(Z)
		//				for(var/obj/NinjaEquipment/TheWires/W in oview(1,src))
		//					if(W)
		...
		Bump(A)
			...
			if(ismob(A))
				...
				step(M,src.dir);
				Pushes+=1
				while(Pushes<PushCap)
					M.DamageProc(src.nin*0.5,"Health",src)
					sleep(4)
				if(Pushes>=PushCap)
					M.Frozen=0
					del(src)
			...
			Bump(A)
				...
				if(ismob(A))
					...
					if(M.Kaiten||M.sphere)
						del(src)
					var/mob/O=src.Owner
					var/damage=round(rand(src.nin*1.63,src.nin*2.4))
					if(M.NibiResistance==1)
						damage /=5
					if(M.NibiResistance==2)
						damage /=7
					...
					if(M.InHydro)
						...
						if(prob(40))
							src.loc=null
							return
					if(src.icon=='Icons/Jutsus/BKatonTechniques.dmi') damage*=1.85
					if(M.PaperStyleDance && !M.InHydro)
						M.PaperStyleDance=0
...
mob/proc/GoukakyuuNoJutsu(Uses,DemonicFlames)
	...
	src.Frozen=1;src.doingG=1;src.InvulnerableToFire=1
	spawn(34)
		src.doingG=0;src.Frozen=0;src.InvulnerableToFire=0;src.overlays-='Icons/Jutsus/KatonTechniques.dmi';src.overlays-='Icons/Jutsus/BKatonTechniques.dmi'
	if(Uses>2000&&src.Charging)
		src.ChakraDrain(275000)
		var/obj/Jutsu/Elemental/Katon/Goukakyuu/K=new();K.Move_Delay=0;K.loc=src.loc;K.nin=src.nin*1.1;K.Owner=src;step(K,src.dir);sleep(1);step(K,src.dir);K.icon_state="5";K.Owner=src;K.nin=src.nin;K.JutsuLevel=src.KatonKnowledge; if(DemonicFlames) K.DemonicFlames=1
		...
		for(var/turf/T in getcircle(K,7))
			var/obj/Jutsu/Elemental/Katon/Goukakyuu/KK=new();KK.Move_Delay=0;KK.loc=T;KK.nin=src.nin*1.1;KK.Owner=src;KK.JutsuLevel=src.KatonKnowledge
	...
	if(src.Inferno==1&&src.HoldingR||src.Inferno>=3&&!src.HoldingR)
		...
		var/obj/Jutsu/Elemental/Katon/Goukakyuu/K=new();K.Move_Delay=0;K.loc=src.loc;K.nin=src.nin*1.25;K.Owner=src;step(K,src.dir);sleep(1);step(K,src.dir);K.icon_state="5";K.Owner=src;K.nin=src.nin*1.2;K.JutsuLevel=src.KatonKnowledge;if(DemonicFlames) K.DemonicFlames=1
		if(src.UchihaMastery>=100)
			K.nin+=25
		var/obj/Jutsu/Elemental/Katon/GoukakyuuTail/Q=new();Q.layer=MOB_LAYER+1
		if(src.dir==SOUTH)
		...
		for(var/turf/T in cardinal(K))
			var/obj/Jutsu/Elemental/Katon/Goukakyuu/KK=new();KK.Move_Delay=0;KK.loc=T;KK.nin=src.nin*1.25;KK.Owner=src;KK.JutsuLevel=src.KatonKnowledge;if(DemonicFlames) KK.DemonicFlames=1
			if(src.UchihaMastery>=100)
				KK.nin+=25
	...
	if(src.Inferno>=3&&src.HoldingR)
		...
		var/obj/Jutsu/Elemental/Katon/Goukakyuu/K=new();K.Move_Delay=0;K.loc=src.loc;K.nin=src.nin*1.25;K.Owner=src;step(K,src.dir);sleep(1);step(K,src.dir);K.icon_state="5";K.Owner=src;K.nin=src.nin*1.2;K.JutsuLevel=src.KatonKnowledge
		if(src.UchihaMastery>=100)
			K.nin+=25
		var/obj/Jutsu/Elemental/Katon/GoukakyuuTail/Q=new();Q.layer=MOB_LAYER+1
		if(src.dir==SOUTH)
...
mob/proc/KatonHousenkaTsumabeniNoJutsu(Uses)
	...
	//		L.loc=get_step(src,turn(src.dir,45))
		if(i==2&&S)
			S.loc=get_step(src,turn(src.dir,-45))
	///		L.loc=get_step(src,turn(src.dir,-45))
		walk(S,src.dir,S.Move_Delay);
	//	walk(L,src.dir,L.Move_Delay)
		if(i==2)
			sleep(4);
		i--;
...
mob/proc/KatonRyuuka()
	...
	var/obj/Jutsu/Elemental/Katon/KRyuuka/K=new(Pwner=src);K.loc = src.loc;K.JutsuLevel=src.KatonKnowledge;K.nin=src.nin*2;K.Move_Delay=0;K.dir = src.dir;K.name="[src]";K.Owner=src;walk(K,src.dir,0,0)
...
mob/proc/KatonRyuusenka()
	...
	src.doingG=1;src.Frozen=1;src.firing=1;spawn(30)
		src.firing=0;src.doingG=0;src.Frozen=0;src.overlays-='Icons/Jutsus/KatonTechniques.dmi'
	while(i>0)
		var/obj/Jutsu/Elemental/Katon/KRyuuka/F=new(Pwner=src);F.loc=src.loc;F.Owner=src;F.dir=src.dir;F.nin=src.nin*2.3;F.Owner=src
...
//	src.icon_state="handseal";
	var/obj/Jutsu/Elemental/Katon/KatonDrill/F=new();F.Owner=src;F.loc=src.loc;F.dir=src.dir;F.nin=src.nin*1.25;F.Owner=src;walk(F,F.dir);F.JutsuLevel=src.KatonKnowledge;sleep(5);src.icon_state=""
	walk(F,F.dir);
...
mob/proc/KatonHouka(Uses)
	...
	src.Handseals(35-src.HandsealSpeed)
	if(src.HandsSlipped) return
	if(Uses>500&&src.HoldingR)
		var/obj/Jutsu/Elemental/Katon/Houka/A=new();A.loc=src.loc;A.Owner=src;A.dir=src.dir;A.nin=src.nin*1.15;A.Owner=src;walk(A,A.dir);A.JutsuLevel=src.KatonKnowledge;sleep(5);src.icon_state=""
//	src.icon_state="handseal";
	var/obj/Jutsu/Elemental/Katon/Houka/F=new();F.Owner=src;F.loc=src.loc;F.dir=src.dir;F.nin=src.nin*1.15;F.Owner=src;walk(F,F.dir);F.JutsuLevel=src.KatonKnowledge;sleep(1);src.icon_state=""
...
mob/proc/ChouHouka(Uses)
	...
	while(i>0)
		...
		if(prob(35))
			sleep(1)
//HoukaInAllDirections
mob/proc/FireFury(Uses)
	if(src.Shibari)
		return
	src.ChakraDrain(35000)
	src.Handseals(53-src.HandsealSpeed)
	...
	if(src.DragonTamer>2)
		Uses*=2
		var/obj/Jutsu/Elemental/Katon/Houka/A=new();A.loc=src.loc;A.Move_Delay=0;A.name="[src]";A.Owner=src;walk(A,NORTH);A.icon_state="5";var/obj/Jutsu/Elemental/Katon/Houka/B=new();B.loc=src.loc;B.Move_Delay=0;B.name="[src]";B.Owner=src;walk(B,SOUTH);B.icon_state="5"
		var/obj/Jutsu/Elemental/Katon/Houka/C=new();C.loc=src.loc;C.Move_Delay=0;C.name="[src]";C.Owner=src;walk(C,WEST);C.icon_state="5";var/obj/Jutsu/Elemental/Katon/Houka/D=new();D.loc=src.loc;D.Move_Delay=0;D.name="[src]";D.Owner=src;walk(D,EAST);D.icon_state="5"
		var/obj/Jutsu/Elemental/Katon/Houka/E=new();E.loc=src.loc;E.Move_Delay=0;E.name="[src]";E.Owner=src;walk(E,NORTHWEST);E.icon_state="5";var/obj/Jutsu/Elemental/Katon/Houka/F=new();F.loc=src.loc;F.Move_Delay=0;F.name="[src]";F.Owner=src;walk(F,SOUTHEAST);F.icon_state="5"
		var/obj/Jutsu/Elemental/Katon/Houka/G=new();G.loc=src.loc;G.Move_Delay=0;G.name="[src]";G.Owner=src;walk(G,SOUTHWEST);G.icon_state="5";var/obj/Jutsu/Elemental/Katon/Houka/K=new();K.loc=src.loc;K.Move_Delay=0;K.name="[src]";K.Owner=src;walk(K,NORTHEAST);K.icon_state="5"
//KatonArmor
mob/proc/Katon_Armor()
	var/X=0
	src.ChakraDrain(28500)
	if(src.sphere||src.Kaiten||src.firing||src.knockedout||src.chakra<=0)
		return
	if(src.DotonArmor&&src.DotonArmor!=2)
		src.overlays-='Icons/Jutsus/RaiArmor.dmi';src.tempmix='Icons/Jutsus/ChakraAura.dmi';src.tempmix+=rgb(224,197,2);var/icon/Q=icon(src.tempmix);src.overlays-=Q;src.DotonArmor=0
	if(src.RaiArmor&&src.RaiArmor!=2)
		src.overlays-='Icons/Jutsus/RaiArmor.dmi';src.tempmix='Icons/Jutsus/ChakraAura.dmi';src.tempmix+=rgb(224,197,2);var/icon/Q=icon(src.tempmix);src.overlays-=Q;src.DotonArmor=0
	src.overlays+='Earth Armor.dmi'
	src.KatonArmor=1
	view()<<"[src] manipulates their earth chakra and forms a armor of fire around their body."
	src.Endurance+=100
	src.FireImmunity+=20
	while(src.KatonArmor&&X<100)
		src.ChakraDrain(8000)
		if(src.chakra<=0)
			src.KatonArmor=0
			src.FireImmunity=5
	...
	src.FireImmunity=5
	src.overlays-='Earth Armor.dmi'
	src<<"Your katon armor begins to dissapear!"

//PyroShit
mob/proc/Asakujaku()
		src.ChakraDrain(10000)
//		src.Effect="Fireball Kick"
obj/Jutsu/Asakujaku
	icon='Icons/Jutsus/Pyro.dmi'
	icon_state="Pyro"
	density=1
	New()
		..()
		overlays+=/obj/Jutsu/Elemental/Katon/F;overlays+=/obj/Jutsu/Elemental/Katon/F;
		spawn(40)
			del(src)
	Bump(A)
		if(ismob(A))
		...
		if(istype(A,/obj/))
			del(src)
	Del()
		src.icon = 'BKatonTechniques.dmi'
		for(var/turf/T in getcircle(src,1))
			var/obj/Jutsu/Elemental/Katon/Goukakyuu/K=new();K.loc=T;K.Owner=src.Owner;..()
mob/var/tmp/AsakujakuOn=0
/*Grand Dragon Fire:
...
//		Uses*=2
	...
	//	src<<"While Loop Called: [A]."
		...
		G.Owner=src;
	//	G.nin=src.nin*10
		G.nin=src.nin*((((src.DragonTamer*(4/3))+5))+1)
		if(src.BlueFire)
