	...
	DotonDD
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(O.BoulderExpert)
					damage*=1.5
				if(O.Trait=="Old Powerful"||O.SecondName=="Bachi")
					var/D=(O.TaiSkill/10)
					if(D<1.1) D=1.1
					damage=damage*D
				if(damage>=5000)
			...
			if(istype(A,/obj/Jutsu/Elemental/))
				...
				if(O.Trait=="Old Powerful"||O.SecondName=="Bachi") damage+=(O.TaiSkill)
			...
			spawn()
				...
				while(src)
					for(var/mob/M in get_step(src,src.dir))
						if(M.Kaiten||M.sphere||M.RaiArmor||M.knockedout) goto Nodamage
						if(M.InHydro)
							src.loc = M.loc
							M.chakra -= 500
							goto Nodamage
		...
		Bump(A)
			...
			if(ismob(A))
				...
				if(src.icon_state=="missle2"||src.icon_state=="firemissle2")
					...
					damage=round(damage*((EarthChakra*0.02)+(src.nin*0.01)))
				if(O.client&&O.Trait=="Old Powerful"||O.SecondName=="Bachi") damage+=(O.NinSkill*7)
				if(damage>=400)
					damage=400
					...
					if(src.icon_state=="DoBind")
						for(var/mob/M in src.loc)
							...
							if(M.SusanooIn)
								...
								del(src)
							if(M.sphere|M.intsu|M.ingat|M.Kaiten|M.RaiArmor|M.knockedout|M.intank|M.cwheel)
								del(src)
							if(M.FrozenBind=="")
					...
					for(var/mob/M in src.loc)
						...
						if(!M.RaiArmor)
							if(M.name!="Crows"&&M.icon!='Crows.dmi'&&M.client||M.name!="Crows"&&M.icon!='Crows2.dmi'&&M.client)
								...
								if(O.Trait=="Old Powerful"||O.SecondName=="Bachi") damage+=O.NinSkill
...
mob/proc
	...
	DotonDoryoDangoz(Uses)
		if(src.buyou||src.Stun>=1|src.DoingHandseals|src.doingG|src.inso|src.Kaiten|src.resting|src.meditating|src.sphere|src.ingat|src.firing|src.FrozenBind!=""|src.doingG||src.Frozen||src.Shibari)
			return
		else
			var/Multiplier=1
			if(src.MudCheck())
			...
			while(src.UsingDomu)
				...
				if(usr.chakra<1)
					for(var/obj/Jutsu/Elemental/Doton/DorouDomu/Objects in view(1,src))
						if(Objects.Owner==src)
							del(Objects)
					src<<"You stop using DorouDomu and release the Earth.";src.icon_state=""
					src.UsingDomu=0
					return
	TsuchiWana()
		src.Target()
	...
	DotonKakouSekkan()
		if(src.firing||src.Kaiten||src.Shibari)
			return
		if(src.UsingKouka)
...
mob/proc/DotonArijigoku()
	if(src.firing||src.Kaiten||src.knockedout||src.Shibari)
		return
	if(src.ntarget)
		return
	src.ChakraDrain(50000)
