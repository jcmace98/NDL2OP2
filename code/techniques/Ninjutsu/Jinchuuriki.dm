obj/Kage_Ring
	...
	verb
		Check_Ring()
			if(src.IchibiSealed==1)
				usr<<"The ring glows bright with the Ichibis chakra.";return
			if(src.NibiSealed==1)
	...
	//		src.loc=locate(usr.x,usr.y-1,usr.z)
		...
		Extract_Bijuu(mob/M in oview(1))
			...
			if(M.Bijuu=="")
				...
				usr.firing=1
				usr.SealingBijuu=1
				usr<<"It'll take 200 seconds to seal the Bijuu!"
				sleep(2000)
				if(M in get_step(usr,usr.dir))
					if(!usr.SealingBijuu)
						return
					usr<<"It'll take 1 more second."
					sleep(10)
					if(M in get_step(usr,usr.dir))
						if(!usr.SealingBijuu)
							return
		...
		Seal_Bijuu(mob/M in oview(1))
			...
			if(M.Bijuu=="")
				switch(input(usr,"Which Bijuu would you like to place into [M]'s body?", "Bijuu") in list ("Ichibi","Nibi","Sanbi","Yonbi","Gobi","Rokubi","Nanabi","Shichibi","Hachibi","Kyuubi"))
					if("Ichibi")
						if(M.Clan=="Sabaku"||M.Clan2=="Sabaku")
							if(src.IchibiSealed==0)
								usr<<"You do not have this Bijuu in your possession!";return
							else
								usr.Frozen=1
								usr.firing=1
								usr.SealingBijuu=1
								usr<<"You begin to seal the Ichibi into [M]'s body, this will take two minutes!"
								sleep(600)
								if(M in get_step(usr,usr.dir))
									if(!usr.SealingBijuu)
										return
									...
									if(M in get_step(usr,usr.dir))
										if(!usr.SealingBijuu)
											return
										if(M.Clan=="Sabaku"||M.Clan2=="Sabaku")
											usr<<"The Ichibi has been sealed into [M]'s body!"
											M.Bijuu="Ichibi"
											src.IchibiSealed=0
											M<<"[usr] has sealed the Ichibi into your body!";return
										else
											usr<<"[M]'s body would not be able to control the Ichibi's abilities.";return
									else
										spawn(300)
											usr.SealingBijuu=0
										usr.Frozen=0
				...
				if(M.Age<15)
					usr<<"They aren't old enough yet..";return
