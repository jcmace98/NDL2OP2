...
obj/EyeThing
	...
	verb
		...
		Implant()
			set src in usr
			usr.Frozen=1
			var/implant=input("Which eye would you like to implant?") in list("Left","Right", "Both","Cancel")
			usr<<"You begin to implant the eye into your socket. Stand still to apply it properly."
			sleep(300)
			var/Chance=15
			if(usr.Subscriber)
				Chance-=15
			if(prob(Chance))
				usr<<"The eyes weren't compatible."
				del(src)
			else
				if(implant=="Left")
					usr.Doujutsu="[Type]"
					usr.HasLeftEye=1
				if(implant=="Right")
					usr.DoujutsuR="[Type] Right"
					usr.HasRightEye=1
				if(implant=="Both")
					if(usr.HasMangekyo)
						usr.EternalMangekyoSharingan=1
					usr.JutsuInLearning="[Type]"
					usr.HasLeftEye=1
					usr.HasRightEye=1
				usr.Frozen=0
				if(usr.mangekyouC=="")
					usr.mangekyouC = "[MangT]"

				usr<<"You sucessfully implanted the eye into your eye socket."
			del(src)
...
obj/DNAThing
	...
	verb
		...
		Implant()
			set src in usr
			usr.Frozen=1
			usr<<"You begin to imprint the DNA into your genetic code. Stand still to apply it properly."
			sleep(300)
			var/Chance=25
			if(usr.Subscriber)
				Chance-=15
			if(prob(Chance))
				usr<<"You imprinted the DNA into you wrong and it overrides your body!"
				...
				world<<"<font color=#8b0000>[usr] attempted to attain the power of another Kekkai Genkai but failed miserably...</font>"
				usr << "Character Deleted"
				del(usr)
			else
				usr.Clan2=Clan
				usr.DeclineAge+=DeclineAge/2
				usr<<"You sucessfully imprinted the DNA into your genetic code."
				del(src)
	...
	spawn(2)
		for(var/mob/M in get_step(src,src.dir))
			if(M.client.computer_id==src.client.computer_id)
				world<<"<font size = 2> [src] tried to take their alts eye/dna. Laugh at the noobtrying to abuse.</font>"
				return
			if(M.client&&src.client&&src.client.address==M.client.address)
				world<<"<font size = 2> [src] tried to take their alts eye/dna. Laugh at the noob trying to abuse.</font>"
				return
			if(M.Dead)
				viewers(4) << "[src] begins to cut into [M]'s eye socket with deadly accuracy!"
				src.firing=1;src.FrozenBind="EyeExtract"
				M.firing=1;M.FrozenBind="EyeExtract"
				var/which=input("Would you like to remove [M]'s eyes?") in list("No","Yes")
				sleep(100)
				M.Bloody()
				spawn(5) M.Bloody()
				var/found=0
				if(which=="No")
					return
				if(which=="Yes")
					if(M.HasLeftEye!=1||M.HasRightEye!=1)
						return;src.firing=0;src.FrozenBind="";M.firing=0;M.FrozenBind="";usr<<"They don't seem to have their eyes.."
					else
						viewers() << "[src] finishes carefully cutting [M]'s eyes out of their sockets!";
						M.HasLeftEye=0
						M.HasRightEye=0
				src.firing=0;src.FrozenBind=""
				M.firing=0;M.FrozenBind=""
				var/obj/EyeThing/E
	...
	spawn(2)
		for(var/mob/M in get_step(src,src.dir))
			if(M.Dead)
				...
				src.firing=1;src.FrozenBind="EyeExtract"
				M.firing=1;M.FrozenBind="EyeExtract"
				var/which=input("Which eye would you like to remove?") in list("No","Yes")
				sleep(100)
				M.Bloody()
				spawn(5) M.Bloody()
				var/found=0
				if("No")
					...
					M.firing=0;M.FrozenBind=""
					return
				if(which=="Yes"&&M.HasLeftEye<1&&M.HasRightEye<1)
					src.firing=0;src.FrozenBind="";M.firing=0;M.FrozenBind="";usr<<"They don't seem to have their eyes..";return
				else
					viewers() << "[src] finishes carefully cutting [M]'s eyes!"
					M.HasLeftEye=0
					M.HasRightEye=0
					src.firing=0;src.FrozenBind=""
					M.firing=0;M.FrozenBind=""
...
mob/proc/ExtractDNA()
	...
	spawn(2)
		for(var/mob/M in get_step(src,src.dir))
			if(M.client.computer_id==src.client.computer_id)
				world<<"<font size = 2> [src] tried to take their alts eye/dna. Laugh at the noob.</font>"
				return
			if(M.client&&src.client&&src.client.address==M.client.address)
				world<<"<font size = 2> [src] tried to take their alts eye/dna. Laugh at the noob.</font>"
				return
			if(M.Dead)
				M.Struggle = 0
				src<<"You begin to extract [M]'s DNA.";M<<"You feel [src] begin to meddle with your body. Something is not right."
				src.firing=1
				src.Frozen=1
				sleep(200)
				src<<"You begin to take blood samples from [M]"
				sleep(50)
				src<<"You've succesfully extracted a part of [M]'s DNA."
				var/obj/DNAThing/E

				for(var/obj/DNAThing/F in M.contents)
					if(F.DeclineAge=="[M.DeclineAge]")
						found=1
						E=F
				if(!found)
					E = new()
					E.DeclineAge=DeclineAge
					E.Clan = M.Clan
					E.name = "[M]'s D.N.A."
					E.loc = src
				src.firing=0
