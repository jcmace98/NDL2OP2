...
*/
	if(src.GateIn=="Wound"||src.GateIn=="Limit"||src.GateIn=="View"||src.GateIn=="Wonder"||src.InDeathGate==1)
...
//Basically saying if it's in the probability of this number.
		...
		if(!src.HandsealAgility)
			src.Frozen=1
		if(prob(10)&&src.HandsealSpeed<=40&&src.HandsealsMastery>=3&&src.Trait=="Speedy")
			src.HandsealSpeed+=pick(0.3,0.5)
		if(prob(10)&&src.HandsealSpeed<=30&&src.HandsealsMastery>=3)
			src.HandsealSpeed+=pick(0.3,0.5)
		if(src.HandsealsMastery<3&&src.NinjutsuKnowledge>10)
			src.HandsealsMastery+=pick(0.3,0.4,0.5,0.7);if(src.HandsealsMastery>3) src.HandsealsMastery=3
		...
		if(HandsealTime>0&&src.Trait=="Speedy")
			HandsealTime*=0.4
		if(HandsealTime>0&&src.CursedSealType=="Air"&&src.CursedSealActivated)
			HandsealTime*=0.45
