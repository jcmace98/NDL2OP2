	...
	Leaf
		...
		Kurama
			...
			Click()
				if(usr.loggedin)
					...
					return
				usr<<sound('SFX/click1.wav',0)
				usr.loc=locate(130,30,1)
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Kurama")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity."
			Description
		...
		Yamanaka
			...
			Click()
				if(usr.loggedin)
					...
					return
				usr<<sound('SFX/click1.wav',0)
				usr.loc=locate(130,52,1)
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Yamanaka")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity."
			Description
		...
		Samurai
			...
			Click()
				if(usr.loggedin)
					...
					return
				usr<<sound('SFX/click1.wav',0)
				usr.loc=locate(110,32,1)
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Mabatsu")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity."
			Description
		...
		Monk
			...
			Click()
				if(usr.loggedin)
					...
					return
				usr<<sound('SFX/click1.wav',0)
				usr.loc=locate(90,32,1)
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Monk")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity."
			Description
		...
		HaitenmaDo
			...
			Click()
				...
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Haitenma-Do")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity."
			Description
		...
		Urase
			...
			Click()
				if(usr.loggedin)
					...
					return
				usr<<sound('SFX/click1.wav',0)
				usr.loc=locate(90,112,1)
				var/count=0
				for(var/mob/M in world)
					if(M.client&&M.Clan=="Urase")
						count++
				if(count>30)
					usr<<"There are currently [count] [src.name] on in case you want diversity."
			Description
...
turf/Finished
	...
	Click()
		...
		if(usr.choseeyes&&usr.baseselected)
			...
			usr.ChakraC=rand(35,55)
			if((prob(5)||usr.Clan=="Fuuma"))
				var/choice=rand(1,3)
				if(choice==1)
			...
			var/B=rand(1,150)
			if(B==1)
				usr.Trait3="Super Taijutsu"
				usr.TaiSkill+=10
			if(B==2)
				usr.Trait3="Super Ninjutsu"
				usr.NinSkill+=10
			if(B==3)
				usr.Trait3="Super Genjutsu"
				usr.GenSkill+=10
			if(B==4)
				usr.Trait3="Super Human Endurance"
				usr.MEndurance+=250;usr.Endurance=usr.MEndurance
			if(B==5)
				usr.Trait3="Super Human Regeneration"
			if(B==6)
				usr.Trait3="Super Human Speed"
			if(B==7)
				usr.Trait3="Prodigy"
			if(usr.Clan=="Uchiha")
				var/Chance=35
				...
				if(prob(Chance))
					...
					var/ASA=list("obito","itachi","madara","sasuke","star","bladed","gridlock","shuriken","shisui","shin")
					var/Mangekyo=pick(ASA);usr.mangekyouC=Mangekyo
			if(usr.Clan=="Basic"||usr.Clan=="Satou"||usr.Clan=="Kiro"||usr.Clan=="Fuuma"||usr.Clan=="Yotsuki")
				var/Chance=10
				if(prob(Chance))
					var/ASA=list("Ichibi","Nibi","Sanbi","Yonbi","Gobi","Rokubi","Shichibi","Hachibi","Kyuubi")
					var/Jinchuuriki=pick(ASA);usr.Bijuu=Jinchuuriki
			...
			if(usr.Clan2=="Shojin")
				...
				if(!(usr.key in Subscribers))
					usr.ElementalPoints-=10
			if(usr.Clan=="Mabatsu")
				usr.LearnedJutsus+= new/obj/SkillCards/ChakraEnhance
				usr.Kenjutsu=120
				usr.SpeedDice=10
				usr.Swift=10
			if(usr.Clan=="Urase")
				usr.CursedSealType="Urase"
				usr.LearnedJutsus+= new /obj/SkillCards/CursedSealActivation
			usr.NameChoose()
			usr.OOC=1
			usr.desc=""
			...
			if(usr.Clan=="Basic")
				switch(usr.SecondName)
					if("Uzumaki")
						...
						usr<<"The Uzumaki Last name is known mostly for Naruto Uzumaki which had a really powerful chakra and lived until 100s of years!"
					if("Hatake")
						...
						usr<<"The Hatake Last name is known mostly for Kakashi Hatake which had a really good control of chakra and he had a really good resistance of stamina!"
					if("Sarutobi")
					...
					if("Kasuke")
						usr.HealthRegen+=rand(40,60)
						usr.StaminaRegen+=rand(50,70)
						usr.Mchakra-=1000;usr.chakra=usr.Mchakra
					if("Loren")
						usr.Mchakra-=2500;usr.chakra=usr.Mchakra
						usr.Mtai+=250;usr.tai=usr.Mtai
					if("Senju")
						usr.Mchakra-=2000;usr.chakra=usr.Mchakra
						usr.Mtai=300;usr.tai=usr.Mtai
						usr.HealthRegen+=rand(40,60)
						usr.StaminaRegen+=rand(50,70)
						usr.maxstamina-=750;usr.stamina=usr.maxstamina
					if("Haruno")
						usr.GenSkill+=rand(4,5)
						usr.ChakraC=100
						usr<<"The Haruno last name is known mostly for Sakura Haruno which has a really good control of Genjutsus and a really Chakra Control!"
					if("Namikaze")
					...
					if("Terumi")
						...
						usr<<"The Terumi Last name is known for its mastery with passives!"
					if("Kurosuki")
						usr.SpeedDice=5
					...
					if("Atsuzawa")
						...
						var/choice=rand(1,3)
						if(choice==1)
							usr.TaiSkill+=rand(1,4)
						if(choice==2)
							usr.NinSkill+=rand(1,4)
						if(choice==3)
							usr.GenSkill+=rand(1,4)
					if("Suikazan")
	...
	//Sound
				...
				if(usr.Clan=="Hyuuga")
					var/ASA=list("Side-Branch","Main-Branch")
					var/ClanBranch=pick(ASA);usr.HyuugaClanBranch=ClanBranch
				...
				var/obj/BonusScrolls/Knowledge_Scroll/F=new();F.loc=usr
				var/obj/BonusScrolls/Knowledge_Scroll/N=new();N.loc=usr
				var/obj/BonusScrolls/Knowledge_Scroll/L=new();L.loc=usr
				switch(input(usr,"This was the Dojo message but idk what to put.. soo...", "YOU WANNA PLAY?") in list ("Yes"))
					if("Yes")
						if(usr.Clan=="Aburame")
							usr.loc=locate(130,183,1)
						else if(usr.Clan=="Akimichi")
							...
							usr.loc=locate(156,150,1)
						else if(usr.Clan=="Kurama")
							usr.loc=locate(156,150,1)
						else if(usr.Clan=="Mabatsu")
							usr.loc=locate(122,130,1)
						else if(usr.Clan=="Monk")
							usr.loc=locate(122,130,1)
			//Sound
						else if(usr.Clan=="Uchiha")
							usr.loc=locate(46,51,21)
							...
							if(usr.Village=="Sound")
								usr.loc=locate(160,60,21)
							if(usr.Village=="Rock")
								usr.loc=locate(84,72,14)
				switch(input(usr,"Have you looked over the Rules? NOTE: You will be held responsible if you break a Rule!", "Rules") in list ("Yes","What Rules?"))
					if("What Rules?")
						usr.RuleGuide()
				if(usr.key in Subscribers)
					usr.Yen+=150000
					...
					while(x<=3)
						...
						x++
						sleep(1)
					usr<<"Thanks for helping support Naruto Destruction Lies!"
...
turf/Customization7
	layer=100
	density=1
	icon='PNG/Screen7.png'
turf/Customization8
	layer=100
	density=1
	icon='PNG/Screen8.png'
turf/Customization9
	layer=100
	density=1
	icon='PNG/RyuukaNotReally.png'
...
mob/proc/FixMyIcon()
	...
	Base+=rgb(src.BaseR,src.BaseG,src.BaseB);src.icon=Base;src.Oicon=src.icon;src.pixel_y=0;src.pixel_x=0
	if(src.Gender=="Male")
