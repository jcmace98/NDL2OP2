				...
				while(src)
					...
					for(var/mob/M in oview(1,src))
						if(!M.Mogu)
							...
							var/damage=rand(M.maxhealth*0.07,M.maxhealth*0.11);damage=round(damage*1.1)
...
/*obj/Jutsu/TriStyhe
	SytheThrow
		icon='Icons/Jutsus/Jashin jutsus.dmi'
		icon_state = "SytheThrowHead"
		density=1
		layer=MOB_LAYER+1
		var
			tai=10
		...
		Move()
			...
			..()
			var/obj/Jutsu/TriSythe/SytheThrowMid/L=new();L.loc=old_loc;L.dir=src.dir;L.Owner=src.Owner
		Bump(A)
			..()
			if(ismob(A))
				var/mob/M=A;var/mob/O=src.Owner;var/damage=O.tai*(390)
				damage=round((damage)*((tai*0.02)+0.9))
			...
			//	M.DamageProc(damage,"Stamina",O)
				M.DamageProc(damage,"Stamina",O,"Sythe","blue")
				M.HitBack(3,src.dir)
				del(src)
			if(istype(A,/turf/))
				var/turf/T = A
			...
			if(istype(A,/obj/))
				if(istype(A,/obj/Doors/))
					var/obj/O=A
					O.DamageProc(src.tai)
					del(src)
				del(src)
		Del()
			for(var/obj/Jutsu/TriSythe/SytheThrowMid/A in world)
				if(A.Owner==src.Owner)
					del(A)
			..()
	SytheTrail
		icon='Icons/Jutsus/Jashin jutsus.dmi'
		icon_state = "SytheThrowMid"
		density=0
		layer=MOB_LAYER+1
		New()
			..()
