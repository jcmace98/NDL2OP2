...
mob/owner/verb/CheckSubscribers()
	set category="Staff"
	usr<<"This is the list of current Subscribers of Naruto Destruction Lies."
...
mob/owner/verb/RemoveSubscription(mob/M in world)
	...
	M.Subscriber=0
	M.SaveK()
	M<<"You have lost your Naruto Destruction Lies Subscription."
	usr<<"You have removed [M]'s Naruto Destruction Lies Subscription."
mob/owner/verb/MakeSubscriber(mob/M in world)
	...
	if(!M.Subscriber)
		M.Subscriber=1
		M<<"You are now a subscriber of Naruto Destruction Lies! Thanks for supporting the game!"
		usr<<"[M] is now a Subscriber of Naruto Destruction Lies!"
		var/Subscribed=M.key
