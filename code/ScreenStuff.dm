...
/*mob/proc/UpdateCards()
	...
	if(!src.controlled)
		...
		for(var/obj/SkillCards/O in src.LearnedJutsus)//Loop through all the items in the players contents
			...
			src << output(O, "Jutsu.Jutsus")//Send the obj's in src.contents to the Grid
		winset(src,"Jutsu.Jutsus", "cells=[X]")
	if(usr.InDeathGate!=1)
				usr.InDeathGate=2
			if(usr.InDeathGate!=2)
				var/Chance=85
				if(usr.Subscriber)
					Chance-=15
				if(prob(Chance))
					usr<<"Your body burns from the inside-out and crumbles to ash"
					...
					world<<"<font color=#8b0000>[usr] has died due to the effects of the death gate...</font>"
					usr << "Character Deleted"
					del(usr)
				else
					usr.GotoVillageHospital()
					usr<<"You were a lucky one of few, and survived the gate of death, only slightly."
					usr<<"Due to it's effects, your body has been severely damaged, permanently."
					usr.maxhealth=750
				usr.GateIn=""
				usr.InDeathGate=0
				usr.icon=usr.Oicon
