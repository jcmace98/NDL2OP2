mob/NPC/RandomNPCs/LeafVillagers
				...
				if(usr.rank=="Genin"&&usr.Clan=="Aburame")
					...
					if(usr.Village=="Leaf")
						if(usr.GenSkill>25&&usr.rank!="Genin")
							...
							usr.LearnJutsu("False Bug Storm",0,"FalseBugStorm","False Bug Swarm is a genjutsu that makes the target actually believe a swarm of bugs is feasting on his flesh and chakra and actually impedes his own movement.")
						if(usr.rank=="Hokage")
							if(prob(50))
								var/style = "<style>BODY {margin:0;font:arial;background:black;\
									color:white;}</style>"
	...
	KusakinJounin1
		...
		verb/Command()
			...
			set hidden=1
			if(src in get_step(usr,usr.dir))
				if(usr.rank=="Student"&&usr.Clan=="Kusakin"||usr.Clan2=="Kusakin")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
	...
	HyuugaJounin1
		...
		verb/Command()
			...
			set hidden=1
			if(src in get_step(usr,usr.dir))
				if(usr.Clan=="Hyuuga"||usr.Clan2=="Hyuuga")
					var/style = "<style>BODY {margin:0;font:arial;background:black;\
						color:white;}</style>"
					if(usr.rank=="Student")
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				...
				for(var/obj/SkillCards/Sharingan/A in usr.LearnedJutsus)
					LearnedSharingan=1
				if(LearnedSharingan&&usr.Clan=="Uchiha"||usr.Clan2=="Uchiha")
	...
	Uchiha_Jack
		...
		verb/Command()
			...
			if(src in get_step(usr,usr.dir))
				...
				usr.alreadytalkingtohim=1
				if(usr.Clan!="Uchiha")
					sd_Alert(usr, "Every generation supposedly five people with intense chakra get the legendary Mangekyo Sharingan...But that's a secret!","[src]",,,,0,"400x150",,style)
				...
				if(usr.Clan=="Shiroi"||usr.Clan2=="Shiroi")
					...
					switch(input(usr,"I can teach you a technique on behalf of your efforts to reach me here, for those who wish to use ice for more destructive purposes... It's an Icy Murder...", "Ice Prison") in list ("Yes","No"))
						if("Yes")
							...
							usr.LearnJutsu("Hyouton: Ice Prison",0,"IcePrison","Ice Prison is a Jutsu not known by many Shirois due to the remaining Shiroi in the Snow Village keeping it secret. You'll have to figure out what it does on your own...","Ninjutsu")
						if("No")
