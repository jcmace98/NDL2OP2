			...
			while(src.health>0)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					for(var/mob/m in oview(5,src))//for all mobs in view(excuding itself)
						if(m.AWA&&m.Village==src.Village&&m.target!=null&&m.target!="")//if M is a NPC in their village and has a target
						...
						else if(m.HasHiddenScroll&&m.Village!=src.Village||!src.walks&&ScrollIsIn==src.Village&&m.Village!=src.Village)
				...
				while(attack&&get_dist(attack,src)<=src.viewsight&&!attack:knockedout&&attack:z==src.z&&!attack:Dead&&!src.Dead)//if its in src's view
					if(src.stamina>50) src.Running=1
					else src.Running=0
					...
					for(var/obj/Jutsu/Elemental/O in view(2,src))
						if(O.JutsuLevel>0&&O.Owner!=src||O.JutsuLevel>0&&O.Owner!=src&&src.FrozenBind==""&&src.Stun<=0&&!src.Frozen)
							step_away(src,O,2)
						...
						if(get_dist(src,attack)==1&&prob(60))
							...
							if(prob(65)&&src.FrozenBind==""&&src.Stun<=0&&!src.Frozen)
								if(prob(60))
									...
									step_to(src,attack)
							else if(src.x==attack:x||src.y==attack:y||(src.x-attack:x)==(src.y-attack:y)||(src.x+attack:x)==(src.y+attack:y))
								if(prob(10))
									var/OkToThrow=1
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					if(src.FrozenBind==""||src.FrozenBind==null)
						step_rand(src)//take a random step
						for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
							if(prob(15)&&m.icon=='Wolf.dmi'&&get_dist(src,m)>3)
								step_to(src,m)
								break
						sleep(src.MoveDelay*5)//wait walkspeed
						for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
							if(m.client&&m.key||m.AOS&&m.icon!='Wolf.dmi')
								piv+=m  //it adds that to people in view)
					else
						sleep(50)
				attack=pick(piv)//pick a rand person 2 atk. remember them.
		...
		AosLoop()
			...
			while(!target)
				...
				while(piv.len<=0&&!src.target)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(src.target==m||m.AOS&&m.Village!=src.Village)
							piv+=m  //it adds that to people in view)
				if(!src.target)
		...
		RockANBU()
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(m.client&&m.key!="SkyAkuto"&&m.Village!=src.Village||m.AOS&&m.Village!=src.Village)//if M is a player or hostile NPC
							piv+=m  //it adds that to people in view)
						if(m.Village==src.Village&&m.client)
							if(findtext(src.talkedtopeople,m.key)==0)
								...
								if(m.rank=="Special Jounin"||m.rank=="Jounin")
									message = pick("Hey there [m.FirstName]!","What's up, [m.FirstName]?","Yo [m.FirstName], we should kick some Leaf puppies sometime soon!","[m.FirstName], don't you just hate the Otokami, Kyrie Eleison?","[m.name], Don't tell anyone, but I heard that Akuto is always watching us...always..")
					...
					if(attack:knockedout&&attack:StruggleAgainstDeath>0)
						...
						while(piv.len<=0)    //if there's nobody in view
							...
							for(var/mob/m in oview(2,src))//for all mobs in view(excuding itself)
								if(m.client&&m.StruggleAgainstDeath<=0&&m.key!="SkyAkuto"&&m.Village!=src.Village||m.AOS&&m.Village!=src.Village&&m.StruggleAgainstDeath<=0)//if M is a player or hostile NPC
									piv+=m  //it adds that to people in view)
						attack=pick(piv)
					if(src.stamina<0)
						var/Drain=abs(src.stamina)
		...
		LeafANBU()
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(m.client&&m.key!="SkyAkuto"&&m.Village!=src.Village&&!m.LeafVillagePermit||m.AOS&&m.Village!=src.Village)//if M is a player or hostile NPC
							piv+=m  //it adds that to people in view)
				attack=pick(piv)//pick a rand person 2 atk. remember them.
				while(get_dist(attack,src)<=viewsight)//if its in src's view
					sleep(1)
...
/*				if(src.HellRealmSummoned)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					if(src.target)
						...
						break
					for(var/mob/m in oview(2,src))//for all mobs in view(excuding itself)
						if(m.client&&m.StruggleAgainstDeath<=0&&m.key!="SkyAkuto"&&src.Owner!=m||m.AOS&&m.StruggleAgainstDeath<=0)//if M is a player or hostile NPC
							if(m.name!=src.name)
								piv+=m  //it adds that to people in view)
				attack=pick(piv)//pick a rand person 2 atk. remember them.
