			...
			if(!src.client)
				...
				if(src.NeedsFace&&src.part1p&&!src.tempmix3)
					if(src.rank=="IDK")
						...
						if(src.Village=="Leaf")
							if(prob(25)){src.Clan="Hyuuga";src.TaijutsuStyle="Jyuken";src.ChakraPercision=5;src.TenketsuAccuracy=20;src.SensoryRange=5;src.Rejection=5}
						if(src.Village=="Sound")
							if(prob(25))
								if(prob(30)){var/obj/WEAPONS/Katana/K=new();src.WeaponInLeftHand=K;src.LeftHandSheath=1}
								src.Clan="Uchiha";src.SharinganMastery=rand(200,1000);src.Reflex=rand(85,450);spawn()
									src.Sharingan()
						src.name="[src.name]([src.rank])"
						if(src.Clan=="Hyuuga"||src.Clan=="Uchiha")
							src.name="[src.Clan] [src.name]([src.rank])"
					if(src.rank=="Jounin")
						if(src.Village=="Leaf")
			...
			while(src.health>0)
				...
				while(attack&&get_dist(attack,src)<=src.viewsight&&!attack:knockedout&&attack:z==src.z&&!attack:Dead&&!src.Dead)//if its in src's view
					...
					if(src.health>src.maxhealth/4&&src.stamina>src.maxstamina/5&&!src.knockedout&&src.Status!="Asleep")
						...
						if(prob(3)&&src.rank=="Chuunin"&&!src.firing)
							spawn() src.Kawa(400)
							step_away(src,attack,4)
						else if(prob(7)&&src.rank=="Jounin"&&!src.firing)
							spawn() src.Kawa2(100)
							step_away(src,attack,4)
						else if(prob(12)&&src.rank=="ANBU")
							spawn(10)
							...
							if(prob(30))
								...
								if(get_dist(src,attack)==1)
									...
									if(prob(40))
										src.AttackVerb3()
							else if(prob(65)&&src.chakra>50&&src.rank!="Genin")
								for(var/obj/SkillCards/S in src.LearnedJutsus)
									if(!S.Delay)
										S.Activate(src)
										break
		...
		RockANBU()
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(m.client&&m.key!=""&&m.Village!=src.Village||m.AOS&&m.Village!=src.Village)//if M is a player or hostile NPC
							piv+=m  //it adds that to people in view)
					...
					if(attack:knockedout&&attack:StruggleAgainstDeath>0)
						...
						while(piv.len<=0)    //if there's nobody in view
							...
							for(var/mob/m in oview(2,src))//for all mobs in view(excuding itself)
								if(m.client&&m.StruggleAgainstDeath<=0&&m.key!=""&&m.Village!=src.Village||m.AOS&&m.Village!=src.Village&&m.StruggleAgainstDeath<=0)//if M is a player or hostile NPC
									piv+=m  //it adds that to people in view)
						attack=pick(piv)
					if(src.stamina<0)
						var/Drain=abs(src.stamina)
		...
		LeafANBU()
			...
			while(src.health>0&&src.z!=20)
				...
				while(piv.len<=0)    //if there's nobody in view
					...
					sleep(src.MoveDelay*5)//wait walkspeed
					for(var/mob/m in oview(src))//for all mobs in view(excuding itself)
						if(m.client&&m.key!=""&&m.Village!=src.Village&&!m.LeafVillagePermit||m.AOS&&m.Village!=src.Village)//if M is a player or hostile NPC
							piv+=m  //it adds that to people in view)
				attack=pick(piv)//pick a rand person 2 atk. remember them.
				while(get_dist(attack,src)<=viewsight)//if its in src's view
				//	sleep(1)
...
mob/owner
	verb
		SummonKyuubiandArgon()
			var/mob/NPC/Argon/O3=new()
			var/mob/NPC/Bijuu/O4=new()
			var/mob/NPC/Boss/O5=new()
			O3.loc=locate(src.x-1,src.y,src.z)
			O4.loc=locate(src.x+1,src.y,src.z)
			O5.loc=locate(src.x+1,src.y,src.z)
			sleep(40)
			KyuubiNPC(O3)
			ArgonProc(O4)
			BossProc2(O5)
