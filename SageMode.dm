mob/EventTrainers/
	GrandmaSage
		...
		verb/Command()
			...
			if(usr.Age>45)
				Check++
			if(usr.rank=="Jounin"||usr.rank=="S.Jounin"||usr.rank=="Sannin")
				Check++
			if(usr.Mchakra>7000)
				Check++
					...
					switch(input(usr,"Hello..", "Would you like to learn the true power of a Medic?") in list ("Yes","No"))
						if("Yes")
							...
							if(usr.BeginningVillage!="Rain")
								...
								return
							usr.LearnJutsu("Sage Mode Slug",0,"SageModeSlug","Sage mode is a technique taught by the eldest medic of the Hidden Rain Village. This technique requires the control over nature chakra in order to manipulate and allows the user to become unbelivably skilled in Medical Ninjutsu","Ninjutsu");return
							usr.Sage+=1
			else
...
mob/EventTrainers/
	Monkey
		...
		verb/Command()
			...
			if(usr.Clan=="Basic")
				switch(input(usr,"Hellooo", "Would you like to be transported to the world of the Sages?") in list ("Yes","No"))
					if("Yes")
						sleep(30)
	...
	Monkey2
		...
		verb/Command()
			...
			switch(input(usr,"Sage Mode is a very difficult and dangerous technique, are you sure you'd risk everything for it..?", "Kokuangyo") in list ("Yes","No"))
				if("Yes")
					if(usr.Sage>0)
						usr<<"You already know the basis of gathering Nature energy.";return
					if(usr.NinjutsuKnowledge<3000)
						usr<<"You need more Ninjutsu Knowledge.";return
					if(usr.Clan!="Basic")
						usr<<"Your bloodline doesn't match the criteria for this technique!";return
					if(usr.SageTest==1)
						usr<<"You haven't completed the test, boy.";return
					usr.LearnJutsu("Sage Ape Mode",0,"SageModeApe","Sage mode is a technique taught by the eldest animal of the type of Sage Mode that is being learnt. This technique requires the control over nature chakra in order to manipulate.","Ninjutsu");return
					usr.Sage=1

	Toad
		name = "Eldest Toad: Fukasaku"
		icon_state="MNPC1"
		icon ='New Icons(Con4)/Fukasaku.dmi'
		CNNPC=1
		health=9999999999999999999999999999999999999999999999
		Village="Leaf"
		verb/Command()
			...
			switch(input(usr,"Sage Mode is a very difficult and dangerous technique, are you sure you'd risk everything for it..?", "Kokuangyo") in list ("Yes","No"))
				if("Yes")
					...
					if(usr.SageTest==1)
						usr<<"You haven't completed the test, boy.";return
					usr.LearnJutsu("Sage Toad Mode",0,"SageModeToad","Sage mode is a technique taught by the eldest animal of the type of Sage Mode that is being learnt. This technique requires the control over nature chakra in order to manipulate.","Ninjutsu");return
					usr.Sage=1
	Snake
		name = "Eldest Snake: Ryuchi"
		icon_state="MNPC1"
		icon ='New Icons(Con4)/MedSnake(White).dmi'
		CNNPC=1
		health=9999999999999999999999999999999999999999999999
		Village="Sound"
		verb/Command()
			...
			switch(input(usr,"Sage Mode is a very difficult and dangerous technique, are you sure you'd risk everything for it..?", "Kokuangyo") in list ("Yes","No"))
				if("Yes")
					...
					if(usr.Village!="Sound")
						usr<<"My allianccccce issss with Sssssound, therefore, I will not teach outssssiders";return
					if(usr.Clan!="Basic")
						usr<<"My technique is only for thossssse that do not already have immenssssse power.";return
					if(usr.NinjutsuKnowledge<5000)
						usr<<"You need more Ninjutsu Knowledge.";return
					usr.LearnJutsu("Sage Toad Snake",0,"SageModeDragon","Sage mode is a technique taught by the eldest animal of the type of Sage Mode that is being learnt. This technique requires the control over nature chakra in order to manipulate.","Ninjutsu");return
					usr.Sage=1
	Lion
		name = "Eldest Sage: Tenma"
		CNNPC = 1
		health = 9999999999999999999999999999999999999999999999
		Village="Rock"
		New()
		...
		verb/Command()
			...
			switch(input(usr,"Sage Mode is a very difficult and dangerous technique, are you sure you'd risk everything for it..?", "Kokuangyo") in list ("Yes","No"))
				if("Yes")
					...
					if(usr.NinjutsuKnowledge<5000)
						usr<<"You need more Ninjutsu Knowledge.";return
					usr.LearnJutsu("Sage Toad Lion",0,"SageModeLion","Sage mode is a technique taught by the eldest animal of the type of Sage Mode that is being learnt. This technique requires the control over nature chakra in order to manipulate.","Ninjutsu");return
					usr.Sage=1
	...
	MonkeyInstructor
		...
		verb/Command()
			...
			var/style = "<style>BODY {margin:0;font:arial;background:black;\
				color:white;}</style>"
			sd_Alert(usr, "If you aren't already aware, this is the sage realm and I'm one of the instructor. You being here must mean you've already been approoved by Moria, in which case he wants you to go through the test. If you manage to pass you may even have the option to learn from our Monkey Elder.Well I've done my part, if you are still interested just head to that building over there and take the test, and good luck boy.","[src]",,,,0,"400x150",,style)
		...
		verb/Command()
			...
			//		usr<<"You've already graduated";return
				...
				switch(input(usr,"Why is nature chakra such an efficent way of usage in combat?","Test",) in list("The nature behind life is used for killing","Its refers to the food chain","The nature chakra uses energy from sourroundings"))
					if("The nature chakra uses energy from sourroundings")
						usr.score+=1
					else
						usr.score+=0
